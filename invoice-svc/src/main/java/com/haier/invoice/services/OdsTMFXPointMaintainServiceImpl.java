 ...
 */
@Component
public  class OdsTMFXPointMaintainServiceImpl implements OdsTMFXPointMaintainService {
    private static final Logger log = LogManager.getLogger(OdsTMFXPointMaintainServiceImpl.class);

    @Autowired
    private OdsTMFXPointMaintainDataService odsTMFXPointMaintainDataService;


    public JSONObject paging (OdsTMFXPointMaintain param, PagerInfo pagerInfo){
        return odsTMFXPointMaintainDataService.paging(param,pagerInfo);
    }
    public JSONObject export (OdsTMFXPointMaintain param){
        return odsTMFXPointMaintainDataService.export(param);
    }
    public ServiceResult<JSONObject> insert (OdsTMFXPointMaintain param){
        ...
        try {
            int count = odsTMFXPointMaintainDataService.insert(param);
            if(count == 1){
    ...
    public ServiceResult<JSONObject> update (OdsTMFXPointMaintain param){
        ServiceResult<JSONObject> serviceResult = new ServiceResult<>();
        try {
            int count = odsTMFXPointMaintainDataService.update(param);
            if(count == 1){
                serviceResult.setSuccess(true);
    ...
    public ServiceResult<JSONObject>  delBatch(List<String> ids){
        ServiceResult<JSONObject> serviceResult = new ServiceResult<>();
        try {
            int count = odsTMFXPointMaintainDataService.delBatch( ids);
            if(count > 0){
                serviceResult.setSuccess(true);
    ...
    public ServiceResult<String> execExcel(List<List<String>> list, String user) {
        ServiceResult<String> result = new ServiceResult<String>();
        StringBuffer info = new StringBuffer();
        List<OdsTMFXPointMaintain> newList = new ArrayList<OdsTMFXPointMaintain>();
        for(int i=0;i<list.size();i++){
            List<String> excel = list.get(i);
            ...
            * */
            ...
            List<OdsTMFXPointMaintain> repetition = odsTMFXPointMaintainDataService.queryRepetition(opm);
        ...
        if (StringUtils.isEmpty(info.toString())){
            try {
                odsTMFXPointMaintainDataService.bulkImport(newList);
    ...
    public String getLogInfo() {
        return odsTMFXPointMaintainDataService.getLogInfo("TM");
    }
}
