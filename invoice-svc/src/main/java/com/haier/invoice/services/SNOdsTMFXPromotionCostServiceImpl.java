...
/**
 * 苏宁分销-营销费用 Created by zdd on 2017/10/23.
 */
@Service("SNOdsTMFXPromotionCostServiceImpl")
public class SNOdsTMFXPromotionCostServiceImpl implements SNOdsTMFXPromotionCostService{

  @Autowired
  private SNOdsTMFXPromotionCostDataService sNodsTMFXPromotionCostDataService;

  /**
   * 分页查询
   */
  @Override
  public JSONObject queryPromptionCost(SNOdsTMFXPromotionCost odsTMFXPromotionCost) {
    return sNodsTMFXPromotionCostDataService.queryPromotionCost(odsTMFXPromotionCost);
  }
   ...
   */
  @Override
  public List<SNOdsTMFXPromotionCost> queryPromptionCostExcel(
		  SNOdsTMFXPromotionCost map) {
    return sNodsTMFXPromotionCostDataService.queryPromotionCostExcel(map);
  }
   ...
   */
  @Override
  public ServiceResult<String> editPromotionCost(SNOdsTMFXPromotionCost odsTMFXPromotionCost) {
    ...
    if (odsTMFXPromotionCost.getId() != 0 && !"".equals(odsTMFXPromotionCost.getId())) {
      //修改
    	sNodsTMFXPromotionCostDataService.updateByPrimaryKeySelective(odsTMFXPromotionCost);
      result.setResult("修改成功");
    } else {
      result = sNodsTMFXPromotionCostDataService.addPromotionCost(odsTMFXPromotionCost);
    }
    return result;
  }

  @Override
  public ServiceResult<String> execExcel(List<SNOdsTMFXPromotionCost> list) {
    return sNodsTMFXPromotionCostDataService.bulkImport(list);
  }
  ...
  public ServiceResult<String> deletePromotionCost(String ids) {
    ...
    try {
      for (String id : setIds) {
    	  sNodsTMFXPromotionCostDataService.deleteByPrimaryKey(id);
      }
  ...
  public ServiceResult<String> auditPromotionCost(String ids, String audit, String nickName) {
    ...
    try {
      if ("B".equals(audit)) {
        //业务初审
        for (String id : setIds) {
          SNOdsTMFXPromotionCost opc = new SNOdsTMFXPromotionCost();
          opc.setId(Integer.parseInt(id));
          opc.setFirstAudit("A");
          opc.setAuditBy(nickName);
          opc.setAuditTime(new Date());
          sNodsTMFXPromotionCostDataService.updateByPrimaryKeySelective(opc);
        }
      }
      if ("F".equals(audit)) {
        //财务复审
