...
import com.haier.logistics.Util.OrderSnUtil;
...
*/
    ...
    public boolean completeCloseOrderProduct(String corderSn, Date signTime) {
        return this.completeCloseOrderProduct(corderSn, signTime, "");
    }

    public boolean completeCloseOrderProduct(String corderSn, Date signTime, String completeType) {
        return completeCloseOrderProductOper(corderSn, signTime, completeType, null);
    }
    public boolean updateOrderWorkflowNetPointArriveTime(Integer orderProductId, Date arriveTime) {
        OrderWorkflows orderWorkflow = orderWorkflowsDao.getByOrderProductId(orderProductId);
        if (orderWorkflow == null) {
            return false;
        }
        Long netPointArriveTime = orderWorkflow.getNetPointArriveTime();
        if (netPointArriveTime != null && netPointArriveTime > 0) {
            return false;
        }
        OrderProductsNew orderProducts = orderProductsDao.get(orderProductId);
        OrdersNew orders = ordersDao.get(orderProducts.getOrderId());
        OrderOperateLogs orderProductOperateLogs;
        DefaultTransactionDefinition def = new DefaultTransactionDefinition();
        def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRED);
        //TransactionStatus status = transactionManager.getTransaction(def);
        try {
            ...
            orderWorkflowsDao.updateNetPointArriveTime(orderWorkflow.getId(),
                    arriveTime.getTime() / 1000);
            if (orderProducts.getStatus() < OrderProductStatus.WAIT_DELIVERY.getCode()) {
                orderProductsDao.updateStatus(orderProducts.getId(),
                        OrderProductStatus.WAIT_DELIVERY.getCode());
            }
            //报表增加更新网单表modified字段，报表使用全流程NetPointArriveTime，但是订单网单没有及时更新
            //            orderProductsDao.updateOpModify(orderProductId);
            //transactionManager.commit(status);
            return true;
        } catch (Exception e) {
            //transactionManager.rollback(status);
            log.error("记录到达网点发生异常，orderSn=" + orderProducts.getCOrderSn(), e);
            return false;
        }
