 ...
 * 2. {@link SysJob#getCfgData()}配置格式如下：
 * 
 * <pre style="margin:0;padding:0;">
 * {
 * interface:"com.haier.cbs.stock.service.OrderThirdCenterStockService",
 * method:"hello",
 ...
 * 如果未指定url参数，则通过注册中心调用Dubbo服务，注册中心配置在cbs-job-service-config.xml中（
 * jobManager的registryProtocol、registryAddress）
 *
...
public class DubboJobLoader extends AbstractJobLoader {
	...
	public JobDetail internalBuildJobDetail() {
		JobDetail jd = newJob(DubboJob.class).withIdentity(
				"[job-" + super.job.getJobType() + "-" + job.getJobId() + "]",
				DEFAULT_GROUP).build();
		...
		if (StringUtil.isEmpty(url, true)) {
			if (log.isInfoEnabled())
				log.info("[job-" + job.getJobId() + "][" + job.getJobName()
						+ "] Dubbo job, using registry: "
						+ super.jobManager.getRegistryProtocol() + "://"
						+ super.jobManager.getRegistryAddress());
			RegistryConfig registryConfig = new RegistryConfig();
			registryConfig.setProtocol("zookeeper");//不可修改名字
			registryConfig.setAddress(job.getZookeeperPath());
	...
	protected void parseCfgData() {
		...
		if (StringUtil.isEmpty(this.interfaceName, true))
			throw new InvalidateCfgDataException(
					"Null or empty dubbo interface name");
		if (StringUtil.isEmpty(this.methodName, true))
			throw new InvalidateCfgDataException(
					"Null or empty dubbo method name");
	}
