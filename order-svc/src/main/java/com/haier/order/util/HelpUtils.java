...
public class HelpUtils {
    ...
    @Autowired
    private OrdersNewService             orderService;
    @Autowired
    private OrderCenterOrderServiceImpl orderThirdCenterOrderService;
    ...
    @Autowired
    private OrderCenterInvoiceNewServiceImpl invoiceService;
    ...
    public String getCustormCode(OrderProductsNew orderProducts, OrdersNew ordersNew, BigDecimal price) {
        ...
        if (!StringUtil.isEmpty(order.getRelationOrderSn(), true)
            ...
            ServiceResult<OrdersNew> rs2 =  orderThirdCenterOrderService.getByOrderSn(order.getRelationOrderSn());
            if (!rs2.getSuccess())
...
//    }
    ...
    public ProductsNew getProducts(Integer productId) {
        ServiceResult<ProductsNew> rs = orderThirdCenterItemService.getProductById(productId);
        if (!rs.getSuccess())
            throw new BusinessException("OrderThirdCenterItemService error:" + rs.getMessage());
        return rs.getResult();
    }
    ...
    public void setOrderService(OrdersNewService orderService) {
...
//    public void setStockCommonService(OrderThirdCenterStockCommonService stockCommonService) {
