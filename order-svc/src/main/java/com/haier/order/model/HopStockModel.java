...
public class HopStockModel {
    ...
    @Autowired
    private BigStoragesRelaService bigStoragesRelaDao;
    @Autowired
    private StoragesRelaService storagesRelaDao;
//    @Autowired
//    private DataSourceTransactionManager transactionManagerShop;
    @Autowired
    private StorageStreetsService storageStreetsDao;
    @Autowired
    ...
    public List<Stock> getStockIncrement(Date dateTime, String channelCode) {
        ...
        if (useHiPeng) {
            List<InvStock> haipstocks = invStockDao.getHAIPStockQty(dateTime,
        ...
        List<InvStock> stockList = invStockDao.getChangedListBySkuWhcode(dateTime,
        ...
        for (InvStock invStock : stockList) {
            ...
            if (useHiPeng && invStock.getSecCode().startsWith("SH")) {
                List<InvStock> haipList = invStockDao.getReliableStockQtyByWhCodesAndSkus(
        ...
        if (useHiPeng) {
            for (Map.Entry<String, InvStock> entryMap : haipMap.entrySet()) {
                ...
                if (entity == null) {
                    List<InvStock> waList = invStockDao.getReliableStockQtyByWhCodesAndSkus(
                        StockBizHelper.stringWithSingleQuote(invStock.getSku()),
    ...
    public List<Stock> getStockListBySku(String sku, String channelCode, boolean notSearchAllflag) {
        ...
        List<InvStock> invStockList = invStockDao.getReliableStockQtyByWhCodesAndSkus(
    ...
    public List<Stock> getStockBySkuAndSecCodes(String sku, List<String> secCodeList,
        ...
        //2017-05-02 净水库存--------END
        List<String> whList = StockBizHelper.convertSecCodesToWh(invSectionDao, secCodeList, useGd,
            useHAIP, channelCode);
    ...
    public List<Stock> getStockList(List<String> skuList, List<String> secCodeList,
        ...
        for (String tempSku : skuList) {
            section = invSectionDao.getBySecCode(secCodeList.get(i));
            List<String> defaultWhs = new ArrayList<String>();
            defaultWhs.add(section.getWhCode());
            // 如果当前库位是上海库，则添加海朋库位
            beforeQueryAddWhs(channelCode, useHAIP, defaultWhs);
            invStockList = invStockDao.getReliableStockQtyByWhCodesAndSkus(
    ...
    private Map<String, Integer> findBackUpStock(boolean useStock, List<String> tempSku,
        ...
        if (useStock) {
            List<InvStock> gdStockList = invStockDao.getReliableStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(tempSku),
                StockBizHelper.stringWithSingleQuote(defaultWhs),
                StockBizHelper.stringWithSingleQuote(stockChannel),
                StockBizHelper.stringWithSingleQuote(channelCode));
    ...
    public Map<Integer, List<String>> getCitySkusByChannel(String channel) {
        ...
        onSaleListResult = orderThirdCenterItemService.getAllOnSaleProducts();
        ...
        startTime = System.currentTimeMillis();
        // 可用渠道查询库存
        List<InvStock> stockList = invStockDao.getStockQtyByWhCodesAndSkus(null, null,
        ...
        // 查询所有城市下的库位列表
        List<StorageCities> citiesList = storageCitiesDao.getStorageCities(null);
        // 查询小家电大家电列表
        List<BigStoragesRela> bigRelaList = bigStoragesRelaDao.getList();
        List<StoragesRela> relaList = storagesRelaDao.getList();
        // 城市下的可用库位
        ...
        Map<String, String> skuSecCodesMap = StockBizHelper.getAllProductssCodes(orderThirdCenterItemService);
        ...
        // 查询库存情况
        List<InvStock> invStockWithSkuList = invStockDao.getStockQtyByWhCodesAndSkus(
    ...
    public Stock getStockBySkuAndRegion(String sku, int regionId, String channelCode,
        ...
        if (addressLevel == 4) {//4级街道查询
            ...
            regionScodeList = storageStreetsDao.getSCodeByStreet(regionId);
            message.append("street(").append(regionId).append("):")
                .append(System.currentTimeMillis() - startTime).append("ms");
        } else {//默认3级区县查询
            regionScodeList = storageCitiesDao.getCodeListByRegion(regionId);
        ...
        startTime = System.currentTimeMillis();
        ServiceResult<ProductBase> productResult = orderThirdCenterItemService.getPorductBaseBySku(sku);
    ...
    private Stock checkStockSmallStoragesRela(boolean isReliable, String sku, String oneToAllCode,
        ...
        List<StoragesRela> smalllalist = storagesRelaDao.getListByCodes(regionCodes);
        ...
        if (stockList == null || stockList.size() == 0) {
            ...
            InvSection section = invSectionDao.getBySecCode(scode);
    ...
    public Stock checkStockBigStoragesRela(boolean checkMultiCode, boolean isReliable, String sku,
        ...
        List<BigStoragesRela> bigRelalist = bigStoragesRelaDao.getListByCodes(regionCodes);
    ...
    public Date stockStatusInCity(String channel, Date startQueryTime) {
        ...
        // 查询WA变化库存
        startTime = System.currentTimeMillis();
        List<InvStock> waChangedList = invStockDao.getChangedListBySkuWhcode(startQueryTime,
            StockBizHelper.stringWithSingleQuote(waChannelList),
            StockBizHelper.stringWithSingleQuote(channel));
        ...
        // 海朋库变化库存
        startTime = System.currentTimeMillis();
        List<InvStock> haipChangedList = invStockDao.getChangedStockQty(startQueryTime,
            StockBizHelper.stringWithSingleQuote(InvSection.CHANNEL_CODE_HAIP),
            StockBizHelper.stringWithSingleQuote(InvSection.CHANNEL_CODE_HAIP));
        ...
        // 基地库变化库存
        startTime = System.currentTimeMillis();
        List<InvStock> gdkChangedList = invStockDao.getChangedStockQty(startQueryTime,
    ...
    private void stockChangedUpdate(List<Stock> waChangedList, List<Stock> haipChangedList,
        ...
        for (Stock entity : waChangedList) {
            ServiceResult<ProductBase> prdService = orderThirdCenterItemService
        ...
        List<StorageCities> allCityIds = storageCitiesDao.getAllCityIds();
...
//            TransactionStatus statusShop = transactionManagerShop.getTransaction(def);
            try {
                ...
                for (Map.Entry<String, Set<String>> entityMap : skuList.entrySet()) {
                    ...
                    for (String sku : value) {
                        ...
                        if (initFlagOn) {
                            ...
                            Integer rows = basChangeStockDao.update(entity);
                            if (rows <= 0) {
                                insertList.add(entity);
                            }
                        }
                ...
                if (insertList.size() > 0) {
                    basChangeStockDao.insert(insertList);
                }
...
//                transactionManagerShop.rollback(statusShop);
    ...
    public Map<String, Set<String>> getStockStatusByCity(String channel, int cityId,
        ...
        List<String> citySCodeList = daStockList.size() > 0
            ? storageCitiesDao.getCodeListByCity(cityId) : new ArrayList<String>();
        startTime = System.currentTimeMillis();

        // 获取大家电多层级关系
        List<BigStoragesRela> daRelaList = daStockList.size() > 0 ? bigStoragesRelaDao.getList()
            : null;
        ...
        if (daStockList != null && daStockList.size() > 0) {
            ...
            for (Stock stock : daStockList) {
                ...
                if (stock.getAvaibleQty() <= 0) {
                    ...
                    List<InvStock> tempdaList = invStockDao.getValidStockQtyByWhCodesAndSkus(
    ...
    private boolean checkZeroHasStock(String sku, List<String> stockChannels, boolean enableWa,
        ...
        if (!tempHasStock && enableWa) {
            ...
            tempWhList.add("WH");
            tempWhList.add("CD");
            List<InvStock> tempXiaoList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(sku),
        ...
        if (!tempHasStock) {
            ServiceResult<ProductBase> prdService = orderThirdCenterItemService.getPorductBaseBySku(sku);
            if (prdService.getSuccess()) {
                ProductBase p = prdService.getResult();
                if (p != null && !StringUtil.isEmpty(p.getScode())) {
                    String sCode = p.getScode();
                    tempStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                        StockBizHelper.stringWithSingleQuote(sku),
        ...
        if (!tempHasStock) {
            List<InvStock> haipStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(sku),
                StockBizHelper.stringWithSingleQuote(InvSection.CHANNEL_CODE_HAIP),
                StockBizHelper.stringWithSingleQuote(InvSection.CHANNEL_CODE_HAIP));
        ...
        if (!tempHasStock) {
            List<InvStock> gdkStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(sku),
                StockBizHelper.stringWithSingleQuote(InvSection.CHANNEL_CODE_GD),
                StockBizHelper.stringWithSingleQuote(InvSection.CHANNEL_CODE_GD));
    ...
    public List<String> getStockStatusByCity(int cityId, String channel) {
        ...
        // 获取上架的商品列表
        List<ProductBase> pList = orderThirdCenterItemService.getAllOnSaleProducts().getResult();
        ...
        if (yiSkuList != null && yiSkuList.size() > 0) {
            ...
            yiStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(skus),
                StockBizHelper.stringWithSingleQuote(whCodes),
                StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()));
        ...
        if (xiaoSkuList != null && xiaoSkuList.size() > 0) {
            ...
            xiaoStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(skus),
                StockBizHelper.stringWithSingleQuote(whCodes),
                StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()));
        ...
        // 根据城市，获取对应的库位列表
        List<String> citySCodeList = storageCitiesDao.getCodeListByCity(cityId);
        ...
        // 获取大家电多层级关系
        List<BigStoragesRela> daRelaList = bigStoragesRelaDao.getList();
        ...
        if (daSkuList != null && daSkuList.size() > 0) {
            ...
            daStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
                StockBizHelper.stringWithSingleQuote(skus),
                StockBizHelper.stringWithSingleQuote(whCodes),
                StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()));
    ...
    private void getSkusByStockChannel(String channel, String stockChannel, List<String> youSkuList,
        ...
        Long totalExecuteTime = 0l;
        Long startTime = System.currentTimeMillis();
        List<InvStock> haipStockList = invStockDao.getValidStockQtyByWhCodesAndSkus(null,
            StockBizHelper.stringWithSingleQuote(channel),
            StockBizHelper.stringWithSingleQuote(stockChannel));
    ...
    public List<String> getStockStatusByCity(int cityId, String channel, Integer itemProperty) {
        ...
        if (yiSkuList != null && yiSkuList.size() > 0) {
            String skus = StockBizHelper.concat(yiSkuList.toArray(new String[yiSkuList.size()]));
            String whCodes = StockBizHelper.concat(yiWhList.toArray(new String[yiWhList.size()]));
            yiStockList = invStockDao.getValidStockQtyByItemProperty(
                StockBizHelper.stringWithSingleQuote(skus),
                StockBizHelper.stringWithSingleQuote(whCodes),
                StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()),
                itemProperty);
        ...
        if (xiaoSkuList != null && xiaoSkuList.size() > 0) {
            ...
            String whCodes = StockBizHelper
                .concat(xiaoWhList.toArray(new String[xiaoWhList.size()]));
            xiaoStockList = invStockDao.getValidStockQtyByItemProperty(
                StockBizHelper.stringWithSingleQuote(skus),
                StockBizHelper.stringWithSingleQuote(whCodes),
                StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()),
                itemProperty);
        ...
        if (daSkuList != null && daSkuList.size() > 0) {
            String skus = StockBizHelper.concat(daSkuList.toArray(new String[daSkuList.size()]));
            String whCodes = StockBizHelper.concat(daWhList.toArray(new String[daWhList.size()]));
            daStockList = invStockDao.getValidStockQtyByItemProperty(
                StockBizHelper.stringWithSingleQuote(skus),
                StockBizHelper.stringWithSingleQuote(whCodes),
                StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()),
                itemProperty);
    ...
    public Stock getStockBySkuAndRegion(String sku, int regionId, String channelCode,
        ...
        Long startTime = System.currentTimeMillis();
        // 根据区县，获取对应的库位列表
        List<String> regionScodeList = storageCitiesDao.getCodeListByRegion(regionId);
        if (regionScodeList == null || regionScodeList.isEmpty()) {
        ...
        try {
            productResult = orderThirdCenterItemService.getPorductBaseBySku(sku);
        } catch (Exception e) {
        ...
        if (!StringUtil.isEmpty(wareHouses)) {
            if (isReliable) {
                stockList = invStockDao.getReliableStockQtyByItemProperty(
                    StockBizHelper.stringWithSingleQuote(sku),
                    StockBizHelper.stringWithSingleQuote(wareHouses),
                    StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()),
                    itemProperty);
            } else {
                stockList = invStockDao.getStockQtyByItemProperty(
                    StockBizHelper.stringWithSingleQuote(sku),
                    StockBizHelper.stringWithSingleQuote(wareHouses),
                    StockBizHelper.stringWithSingleQuote(stockChannel.getStockChannelCodes()),
                    itemProperty);
    ...
    private void subtractLockQty(List<Stock> afterStockList) {
        if (afterStockList != null && afterStockList.size() > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                List<InvStockOrderLockEntity> tempLockList = invStockOrderLockDao
