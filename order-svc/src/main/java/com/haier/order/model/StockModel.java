...
import com.haier.order.helper.StockBizHelper;
...
//    private DataSourceTransactionManager transactionManagerShop;
    @Autowired
    private StockInvBaseStockService invBaseStockDao;
    @Autowired
    ...
    @Autowired
    private StockInvStockBatchService         invStockBatchDao;
    @Autowired
    private InvStockService              invStockDao;
    @Autowired
    private InvBaseStockLogService       invBaseStockLogDao;
    @Autowired
    private InvLesStockService           invLesStockDao;
    private StorageProductsService       storageProductsDao;
    private InvSgStockService      invSgStockDao;
    private InvStock2ChannelService      invStock2ChannelDao;
    private InvDbmBaseService           invDbmBaseDao;
    private InvStockQtyDifLogService     invStockQtyDifLogDao;
    private SgRealtimeStockService sgRealtimeStockDao;
    private InvDbmBaseSendAddressService invDbmBaseSendAddressDao;

    private SgFlagShipStoreAuthorizationService sgFlagShipStoreAuthorizationDao;
    private InvBaseStockDiffService invBaseStockDiffDao;
    private OrderCenterLESServiceImpl orderThirdCenterLesService;
    private InvBaseStockDiffLogService invBaseStockDiffLogDao;
    ...
    public ServiceResult<Boolean> frozeStockQty(String sku, String secCode, String refNo,
        ...
        //检查单据号是否已经冻结
        List<InvStockLock> locks = invStockLockDao.getNotReleasedByRefNo(refNo);
        if (locks.size() > 0) {
            int qty = 0;
...
//        TransactionStatus statusShop = transactionManagerShop.getTransaction(def);
        try {
            ...
            for (Entry<String, InvStock> stockEntry : skuStockMap.entrySet()) {
                stockSection = this.invSectionDao.getBySecCode(stockEntry.getValue().getSecCode());
                StockBizHelper.linkageUpdateStockForSales(orderThirdCenterItemService, invSectionDao, invStockDao,
                    invStockChannelDao, stockChangeQueueDao, invStockChangeDao,
...
//            transactionManagerStock.rollback(status);
    ...
    public ServiceResult<String> frozeStockQtyByWhCode(String sku, String whCode,
        ...
        try {
            locks = invStockLockDao.getNotReleasedByRefNo(refNo);
        } finally {
        ...
        for (String stockChannelCode : stockChannelList) {
            ...
            List<InvSection> _sections = invSectionDao.getByChannelCode(tempWhCode,
                stockChannelCode);
        ...
        for (Entry<String, List<InvSection>> entry : sections.entrySet()) {
            ...
            for (StockHolder holder : stockHolders) {
                ...
                for (InvSection section : _sections) {
                    InvBaseStock baseStock = (InvBaseStock) invBaseStockDao.get(holder.getSku(),
                        section.getSecCode());
...
//            transactionManagerStock.rollback(status);
    ...
    private List<InvStock> regroupStockQtyBySku(String sku, String secCode, Integer stockQty) {
        ...
        List<InvMachineSet> machineSets = invMachineSetDao.getBySubSku(sku);
        if (machineSets.size() <= 0) {
            ...
            for (InvMachineSet machineSet : machineSets) {
                ...
                List<InvMachineSet> bomInfos = invMachineSetDao.getByMainSku(mainSku);
    ...
    private void calculateBomStockByBase(InvStock invStock, List<InvMachineSet> bomInfos) {
        ...
        for (InvMachineSet bominfo : bomInfos) {
            ...
            InvBaseStock subStock = (InvBaseStock) invBaseStockDao.get(subSku,
                invStock.getSecCode());
            if (subStock == null) {
                minStock = 0;
                break;
    ...
    public ServiceResult<Boolean> releaseFrozenStockQty(String sku, String refNo,
        ...
        for (StockHolder stockHolder : stockHolders) {
            List<InvStockLock> stockLocks = invStockLockDao.getNotReleasedByRefNoSku(refNo,
                stockHolder.getSku());
...
//            transactionManagerStock.rollback(status);
    ...
    private void updateStockLockAfterReleaseQty(String sku, String sec_code, String refNo,
    	...
    	InvStockLock stockLock = invStockLockDao.getNotReleased(refNo, sku, sec_code);
    	if (stockLock == null) {
    		...
    		if (notReleaseQty >= releaseQty) {
    			Integer effectNum = invStockLockDao.updateReleaseQty(stockLock.getId(), releaseQty,
    					optUser);
    			if (effectNum >= 1) {
    				isSuccess = true;
    			}
    ...
    private InvBaseStock releaseBaseStockQty(String sku, String secCode, String refNo,
        ...
        Date time = new Date();
        Integer effectNum = invBaseStockDao.releaseQty(baseStock.getStoId(), releaseQty, time);
        if (effectNum < 1) {
        ...
        invBaseStockLogDao.insert(baseStockLog);
        return (InvBaseStock) invBaseStockDao.get(sku, secCode);
    }
    ...
    private void assignmentValue2InvBaseStockLog(InvBaseStockLog baseStockLog) {
        ...
        InvSection invSection = invSectionDao.getBySecCode(baseStockLog.getSecCode());
        if (invSection != null) {
    ...
    private ItemBase getItemBaseBySku(String sku) {
        ServiceResult<ItemBase> result = orderThirdCenterItemService.getItemBaseBySku(sku);
    ...
    public Boolean checkIfSkuHasStock(String sku) {
        ...
        //获取所有商品的库存
        List<InvStock> stockList = invStockDao.getValidStockQtyByWhCodesAndSkus(
    ...
    public ServiceResult<Boolean> updateCbsStockByStockTransaction(InvStockTransaction stockTransaction) {
        ...
        InvStockLock stockLock = invStockLockDao.getNotReleased(stockTransaction.getCorderSn(),stockTransaction.getSku(), stockTransaction.getSecCode());
        ...
        InvSection section = invSectionDao.getBySecCode(stockTransaction.getSecCode());
        if (section == null) {
            result.setResult(false);
...
//        TransactionStatus status = transactionManagerStock.getTransaction(def);
        try {
            ...
            if (frozeQty > 0) {
                if (stockLock != null) {
                    invStockLockDao.updateLockQty(stockLock.getId(), frozeQty, "sys");
                    Integer oldFrozenQty = baseStock.getFrozenQty();
                    ...
                    int effectRows = invBaseStockDao.updateQtyForFrozen(baseStock);
                    if (effectRows < 1) {
                        throw new BusinessException("库存不足，占用失败");
                    }
                    invStockLockDao.updateLockQty(stockLock.getId(),
            ...
            if (isEnterInW10) {
                ...
                for (InvStock invStock : stockList) {
                    StockBizHelper.linkageUpdateStockForSales(orderThirdCenterItemService, invSectionDao,
                        invStockDao, invStockChannelDao, stockChangeQueueDao, invStockChangeDao,
...
//            transactionManagerStock.rollback(status);
    ...
    private void updateStockBatch(String sku, String secCode, String refNo,
        ...
        if ("S".equals(mark)) {//入库,自动新增批次，不考虑多次入库一个批次的情况
            ...
            String lastBatchNum = invStockBatchDao.getLastBatchNum();
            ...
            invStockBatchDao.insert(stockBatch);
        } else if ("H".equals(mark)) {//出库
            while (qty > 0) {
                Integer _qty;
                InvStockBatch stockBatch = invStockBatchDao.getFrontAvailable(sku, secCode);
                if (stockBatch == null) {
                    return;
                }
                ...
                invStockBatchDao.updateQty(stockBatch);
            }
        }
    }
    /**
    ...
    private InvBaseStock updateStockBase(String sku, String lesSecCode, String secCode, Integer qty,
        ...
        if (isInsert) {
            ...
            Integer n = invBaseStockDao.updateQty(baseStock.getStoId(), signedQty, releaseQty, now);
            if (n <= 0) {
                throw new BusinessException("数据错误或者当前库存不满足");
            }
        }

        baseStock = (InvBaseStock) invBaseStockDao.get(sku, secCode);//重新获取记录
    ...
    private void assignmentValue2InvBaseStock(InvBaseStock baseStock) {
        ...
        InvSection invSection = invSectionDao.getBySecCode(baseStock.getSecCode());
        if (invSection != null) {
            String secName = invSection.getSecName();//库位名称
    ...
    private ItemAttribute getItemAttributeByValueSetIdAndValue(String valueSetId, String value) {
        ServiceResult<ItemAttribute> result = orderThirdCenterItemService
...
//        TransactionStatus statusShop = transactionManagerShop.getTransaction(def);
        try {
            ...
            InvLesStock lesStock = invLesStockDao.getBySecCodeAndSku(secCode, sku);
            if (lesStock == null) {
                lesStock = new InvLesStock();
                lesStock.setSecCode(secCode);
                lesStock.setSku(sku);
                lesStock.setStockQty(newQty);
                invLesStockDao.insert(lesStock);
            } else {//更新库存
                lesStock.setStockQty(newQty);
                invLesStockDao.update(lesStock);
            }
            //同步日日顺可用库存
...
//            transactionManagerStock.rollback(status);
    ...
    private void syncRRSStockQtyFromLes(String sku, String secCode, Integer newQty) {
        ...
        InvBaseStock baseStock = invBaseStockDao.getForUpdate(sku, secCode);
        boolean isInsert = false;
        if (baseStock == null) {
            isInsert = true;
            baseStock = new InvBaseStock();
        ...
        if (isInsert)
            ...
            invBaseStockDao.updateStockQty(baseStock.getStoId(), newStockQty, dateTime);
        //直接是可以数
        List<InvStock> stockList = regroupStockQtyBySku(sku, secCode, baseStock.getStockQty());
        //同步库存到商城
        InvSection stockSection;
        for (InvStock stock : stockList) {
            stockSection = this.invSectionDao.getBySecCode(stock.getSecCode());
            //联动更新销售库存信息
            StockBizHelper.linkageUpdateStockForSales(orderThirdCenterItemService, invSectionDao, invStockDao,
                invStockChannelDao, stockChangeQueueDao, invStockChangeDao, stock, stockSection,
                true);
        ...
        try {
            for (StockHolder holder : stockHolderList) {
                ...
                if (newStockQty < 0)
                    newStockQty = 0;
                //查找库位
                InvSection section = invSectionDao.getBySecCode(holder.getSecCode());
                //不存在添加
                if (section == null) {
...
//        TransactionStatus statusShop = transactionManagerShop.getTransaction(def);
        ...
        try {
            ...
            for (StockHolder holder : stockHoldersForOut) {
                //查询库位信息
                section = this.invSectionDao.getBySecCode(holder.getSecCode());
                //出库操作
...
//            transactionManagerStock.rollback(status);
    ...
    private InvBaseStock updateCbsStockByInventoryBusiness(String sku, InvSection section,
        ...
        if (releaseQty > 0) {//确定基础库存冻结数量
            InvStockLock stockLock = invStockLockDao.getLast(refNo, sku, section.getSecCode());
            if (stockLock == null) {
                baseReleaseQty = 0;
    ...
    public boolean saleSubMachine(String sku) {
        ...
        Integer rows = invStockDao.addSaleSubMachines(sku);
        return rows > 0;
    }
    ...
    public boolean deleteSaleSubMachine(String sku) {
        ...
        Integer rows = invStockDao.deleteSaleSubMachines(sku);
        return rows > 0;
    }
    /**
     * 根据物料编码、库位获取CBS库存
    ...
    public InvStock getInvStock(String sku, String secCode) {
        return this.invStockDao.getBySecCodeAndSku(secCode, sku);
    }
    ...
    public List<StorageProducts> getAllEffectiveLocks() {
        return storageProductsDao.getAllEffectiveLocks();
    }
    ...
    public StorageProducts getStorageProducts(String sCode, String sku) {
        return storageProductsDao.getBySCodeAndSku(sCode, sku);
    }
    ...
    public List<InvStockBatch> queryInvStockBatchList(Integer lastBatchId, int startIndex,
                                                      int pageSize) {
        return this.invStockBatchDao.queryInvStockBatch(lastBatchId, startIndex, pageSize);
    }
    ...
    public String compareStockQtyDifLog() {
        ...
        List<InvStockQtyDifLog> difList = this.invStockQtyDifLogDao.queryDifStockQty();
        msg.append("本次差异数据有").append(difList.size()).append("条");
        int errorCnt = 0;
        for (InvStockQtyDifLog difLog : difList) {
            try {
                difLog.setCreateUser("[job]");
                this.invStockQtyDifLogDao.insert(difLog);
            } catch (Exception e) {
                errorCnt++;
    ...
    public List<InvDbmBase> selectInvDbmBase(Map<String, Object> params) {
        ...
        try {
            result = invDbmBaseDao.selectInvDbmBaseItem(params);
        } catch (Exception ex) {
    ...
    public List<String> findAllBaseStorage(Map params) {
        ...
        try {
            result = invDbmBaseDao.findAllBaseStorage(params);
        } catch (Exception ex) {
    ...
    public List<InvDbmBaseSendAddress> selecInvDbmBaseSendAddress(Map<String, Object> params) {
        ...
        try {
            result = invDbmBaseSendAddressDao.selectInvDbmBaseSendAddressItem(params);
        } catch (Exception ex) {
    ...
    public Date syncSgStockFromEisToShop(Date lastSyncDate) {
        ...
        //获取变化的库存记录
        List<InvSgStockEntity> stocks = invSgStockDao.fingSgStockByLastTime(lastSyncDate, 5000);
        if (stocks.size() <= 0) {
        ...
        do {
            ...
            for (InvSgStockEntity stock : stocks) {
                ...
                String storeCode = sgRealtimeStockDao
    ...
    public Integer autoSynchLesStock(Integer maxId, Integer maxNum) {
        List<InvBaseStockDiff> lists = invBaseStockDiffDao.queryInvBaseStockDiff(maxId, maxNum);
        if (lists != null && !lists.isEmpty()) {
            for (InvBaseStockDiff stockDiff : lists) {
                try {
                    ...
                    InvBaseStock baseStock = invBaseStockDao.queryBySkuAndLesSecCode(query.getSku(),
                        query.getSecCode());
                    if (baseStock != null) {
                        stockDiff.setStockQty(baseStock.getStockQty());
                        stockDiff.setFrozenQty(baseStock.getFrozenQty());
                    }
                    ServiceResult<QueryLesStockOutType> res = orderThirdCenterLesService.queryLesStock(query);
                    //DefaultTransactionDefinition def = new DefaultTransactionDefinition();
                    //def.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRED);
                    //TransactionStatus status = transactionManagerStock.getTransaction(def);
    ...
    public Boolean autoDeleteInvBaseStockDiffLog() {
        try {
            String maxDate = invBaseStockDiffLogDao.getMaxTime();
            if (maxDate == null) {
                return false;
            }
            if (maxDate != null) {
                invBaseStockDiffLogDao.batchDelete(maxDate, max_day);
            }
            return true;
        } catch (Exception e) {
    ...
    public List<SgFlagShipStoreAuthorization> syncToRegionEc(Integer storeId,
		...
		try {
			lists = sgFlagShipStoreAuthorizationDao.queryByCondition(storeId,
					brandId, department);
		} catch (Exception e) {
...
//    }
    ...
    private InvBaseStockDiff updateInvBaseStockDiff(CBSKCType type, InvBaseStockDiff stockDiff) {
        ...
        invBaseStockDiffDao.updateInvBaseStockDiff(stockDiff);
        return stockDiff;
    }
    private void insertInvBaseStockDifLog(InvBaseStockDiff stockDiff) {
        ...
        invBaseStockDiffLogDao.insert(diffLog);
    }
}
