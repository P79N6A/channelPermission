...
import com.haier.order.util.StrTools;
import com.haier.order.util.UnmarshalXmlUtil;
...
//    private DataSourceTransactionManager   transactionManager;
    ...
    @Autowired
    private OrderCenterInvoiceToTaxServiceImpl orderThirdCenterInvoiceToTaxService;
    @Autowired
    private OrderCenterEInvoiceServiceImpl orderThirdCenterEInvoiceService;
    @Autowired
    private OrderCenterEInvoiceV2ServiceImpl eInvoiceV2ServiceImpl;
    @Autowired
    ...
    private void eInvoiceSendPhp(Invoices invoices, InvoiceElectricLogs eInvoiceLog) {
        switch (eInvoiceLog.getType()) {
            case 1:
                ...
                break;
            case 2:
                invoices.setStatusType((int) 4);
                invoices.setTryNum(0);
                invoices.setSuccess((int) 0);
                invoicesDao.update(invoices);
                break;
            default:
                break;
    ...
    public void invalidInvoice(Invoices invoices) throws Exception {
        ...
        try {
            invalidInvoiceToTaxSys = orderThirdCenterInvoiceToTaxService.invalidInvoiceToTaxSys(invoiceEntity);
        } catch (Exception e) {
    ...
    public void editInvoice(Invoices invoices, Integer status, Integer orderId) throws Exception {
        ...
        try {
            modifyInvoiceToTaxSys = orderThirdCenterInvoiceToTaxService.modifyInvoiceToTaxSys(inputType);
        } catch (Exception e) {
...
//                    transactionManager.rollback(status);
    ...
    private Invoices AssemblyInvoiceInfo(OrdersNew order, OrderProductsNew op, MemberInvoices mi) {
        ...
        //获取产品对象
        ProductsNew product = orderThirdCenterItemService.getProductBySku(op.getSku()).getResult();
        if (product == null) {
            log.error("组装发票信息：产品不存在，sku（" + op.getSku() + "）");
            return null;
        }
        //获取产品类型
        ProductTypesNew productType = orderThirdCenterItemService.getProductType(product.getProductTypeId())
                .getResult();
        ...
        invoice.setCOrderType((int) 1);// 普通正常网单
        invoice.setDiffId(0);//
        ...
        invoice.setType((int) mi.getInvoiceType().byteValue());//1增票，2电子，3普票（普税纸质）2015-09-28
        invoice.setIsTogether((int) isTogether.byteValue());
        invoice.setState((int) 0);
        ...
        invoice.setBillingTime((long) 0);//0l
        invoice.setEaiWriteTime((long) 0);//0l
        invoice.setDrawer("");
        invoice.setEaiWriteState("");
        invoice.setInvalidTime((long) 0);//0l
        ...
        invoice.setStatusType((int) 1);//首次推送
        invoice.setSuccess((int) 0);
        invoice.setAddTime((long) Integer.parseInt(Ustring.getString(new Date().getTime() / 1000)));
        invoice.setTryNum(0);
        ...
        invoice.setElectricFlag((int) (mi.getElectricFlag().toString() != null && mi.getElectricFlag().equals(1) ? 1
    ...
    public void syncToInvoiceSystemThread(List<Invoices> list) {
        if (list != null && list.size() > 0) {
            ...
            for (Invoices invoices : list) {
                ...
                try {
                    ReadWriteRoutingDataSourceHolder.setIsAlwaysMaster(Boolean.TRUE);
                    Integer statusType = invoices.getStatusType();
                    if (statusType == null) {
                        continue;
                    }
                    switch (statusType) {
        ...
        try {
            ...
            if (invoices.getElectricFlag().equals(1)) {
                ...
                try {
                    ...
                    result = orderThirdCenterInvoiceToTaxService.createInvoiceToTaxSys(invoiceEntity);
                } catch (Exception e) {
...
//                TransactionStatus status = transactionManager.getTransaction(def);
                try {
                    if (result == null) {
                        ...
                        invoices.setState((int) 1);
                        invoices.setSuccess((int) 1);
                    }
    ...
    public void syncToInvoiceSystemCreateOld(Invoices invoices) {
        ...
        try {
            ...
            if (memberInvoices.getElectricFlag().equals(1) || invoices.getElectricFlag().equals(1)) {
                ...
                // 生成发票推送日志
                if (!(invoices.getElectricFlag() != null && invoices.getElectricFlag().equals(1))) {
                    invoices.setElectricFlag((int) InvoiceConst.ELECTRIC_INVOICE.byteValue());
                    invoicesDao.update(invoices);//后面还会执行更新
                }
                OrdersNew order = null;
                if (InvoiceConst.COMMON_CORDER_TYPE.equals(invoices.getCOrderType())) {
...
//                    transactionManager.rollback(status);
    ...
    public void syncToInvoiceSystemUpdate(Invoices invoice) {
        ...
        try {
            //请求金税更新
            ServiceResult<InvoiceOutType> result = orderThirdCenterInvoiceToTaxService
            ...
            this.writeInvoiceApiLogs(invoice.getId(), InvoiceConst.EDIT_KIND,
                    ...
                    pushData, null);
            //更新Invoice
            invoice.setSuccess((int) isSucess.byteValue());
            invoice.setTryNum((invoice.getTryNum() == null ? 1 : invoice.getTryNum()) + 1);
            invoicesDao.update(invoice);
        } catch (Exception e) {
            this.writeInvoiceApiLogs(invoice.getId(), InvoiceConst.EDIT_KIND,
    ...
    public void syncToInvoiceSystemCancel(Invoices invoice) {
        ...
        try {
            ...
            this.writeInvoiceApiLogs(invoice.getId(), InvoiceConst.CANCEL_KIND,
                    ...
                    pushData, null);
            //更新发票状态
            invoice.setState((int) InvoiceConst.CANCEL_STATE.byteValue());//已取消开票
            invoice.setSuccess((int) isSucess.byteValue());
            invoice.setTryNum((invoice.getTryNum() == null ? 1 : invoice.getTryNum()) + 1);
            invoicesDao.update(invoice);
        } catch (Exception e) {
    ...
    public void syncToInvoiceSystemInvalidOld(Invoices invoices) {
        ...
        if (invoices.getElectricFlag().equals(1)) {// 电子发票系统
            ...
            try {
                ...
                result = orderThirdCenterInvoiceToTaxService.invalidInvoiceToTaxSys(invoiceEntity);
            } catch (Exception e) {
        ...
        switch (type) {
            case 1://开票
                ...
                try {
                    ...
                    //调用电子开票接口
                    result1 = orderThirdCenterEInvoiceService.createInvoice(invoiceElectricLogs.getPushData(),
...
//                    transactionManager.rollback(transtatus);
            ...
            case 2://作废
                ...
                try {
                    //调用电子开票作废接口
                    result2 = orderThirdCenterEInvoiceService.invalidInvoice(invoiceElectricLogs.getPushData(),
                            invoiceElectricLogs.getOrderProductId().toString());
                } catch (Exception e) {
...
//                    transactionManager.rollback(transtatus2);
    ...
    private boolean eInvoiceUpdateOld(Invoices invoice, Integer type, Map<String, String> attMap) {
        ...
        switch (type) {
            case 1://开票
                invoice.setState((int) InvoiceConst.COMPLETE_STATE.byteValue());
                invoice.setSuccess((int) 1);
                invoice.setEaiWriteState(InvoiceConst.NORMAL_KP_STATE);
                invoice.setDrawer("CBS");
                invoice.setBillingTime((long) Integer.parseInt(attMap.get("generateTime")));
                ...
                InvoiceSAPLogs invoiceSAPLogs = OrderCenterOrderBizHelper.sapGenerate(invoiceSAPLogsDao,
            ...
            case 2://作废
                ...
                if (attMap.get("status").equals("3") || attMap.get("status").equals("4")) {//原php invalidateType=1   新java  status=3冲红     status=2作废   status=1开票   status=4被红冲
                    // 红冲
                    invoice.setInvalidTime((long) Integer.parseInt(attMap.get("validTime")));
                ...
                invoiceSAPLogs = OrderCenterOrderBizHelper.sapGenerate(invoiceSAPLogsDao, orderProductsDao,
                ...
                /*boolean flag2 = OrderCenterOrderBizHelper.sapGenerate(invoiceSAPLogsDao, orderProductsDao,
                    invoice, 0, this.getPathByName("/upfiles/sapblacklist.txt"));
                if (!flag2) {
...
//                transactionManager.rollback(transtatus);
    ...
    public ServiceResult<Response> queryInvoice(String COrderSn) {//此方法已不再使用
        ...
        //log.info("====================发送的xml：====================\n" + queryXml);
        //        ServiceResult<String> result = orderThirdCenterEInvoiceService.queryInvoice(queryXml, COrderSn);
                ...
                for (Invoices invoices : list) {
                    ...
                    try {
                        ...
                        if (paramMap != null) {
                            ...
                            if (!OrderCenterOrderBizHelper.eInvoiceEntityToInvalidInvoicesOld(paramMap,
                                    invoices)) {
...
//                            TransactionStatus status = transactionManager.getTransaction(def);
                            try {
                                if (OrderCenterOrderBizHelper.updateInvoiceInfo(invoices, 2, invoicesDao,
                ...
                for (Invoices invoices : list) {
                    ...
                    try {
                        ...
                        if (paramMap != null) {
                            //                            Response response = electricResult.getResult();
                            //                            Invoice invalidInvoice = response.getInvoice();
                            if (!OrderCenterOrderBizHelper.eInvoiceEntityToInvalidInvoices(paramMap, invoices)) {
                                invoices.setTryNum((invoices.getTryNum() == null ? 1 : invoices
                                        .getTryNum()) + 1);
                                invoicesDao.update(invoices);
                                continue;
                ...
                for (Invoices invoices : list) {
                    ...
                    try {
                        ...
                        if (paramMap != null) {
                            ...
                            if (!OrderCenterOrderBizHelper.eInvoiceEntityToInvoicesOld(paramMap, invoices)) {
                                invoices.setTryNum((invoices.getTryNum() == null ? 1 : invoices
                                        .getTryNum()) + 1);
                                invoicesDao.update(invoices);
                                continue;
                            }
                            Integer pushType = OrderCenterOrderBizHelper.getInvoiceSAPLogsOfPushType(invoices,
                                    orderProductsDao);
...
//                            TransactionStatus status = transactionManager.getTransaction(def);
                            try {
                                if (OrderCenterOrderBizHelper.updateInvoiceInfo(invoices, pushType,
                ...
                for (Invoices invoices : list) {
                    ...
                    try {
                        ...
                        if (paramMap != null) {
                            //                            Response response = electricResult.getResult();
                            //                            Invoice eInvoice = response.getInvoice();
                            if (!OrderCenterOrderBizHelper.eInvoiceEntityToInvoices(paramMap, invoices)) {
                                invoices.setTryNum((invoices.getTryNum() == null ? 1 : invoices
                                        .getTryNum()) + 1);
                                invoicesDao.update(invoices);
                                continue;
