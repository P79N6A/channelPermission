...
import com.haier.order.services.OrderCenterStockAgeServiceImpl;
...
public class StockTransactionModel<T> {
    ...
    @Autowired
    private OrderCenterStockServiceImpl stockService;
    @Autowired
    private OrderCenterStockAgeServiceImpl stockAgeService;
    @Autowired
    private OrderCenterTransferLineServiceImpl transferLineService;
    @Autowired
    private StockInvStockTransactionService       invStockTransactionDao;
//    @Autowired
//    private DataSourceTransactionManager transactionManagerStock;
    ...
    @Autowired
    private StockStockTransactionExistService     stockTransactionExistDao;
    ...
    public boolean addStockTransaction(InvStockTransaction stockTransaction,
        ...
        if (lesStockTransQueueId != null) {
            StockTransactionExist exist = stockTransactionExistDao.getByLesStockTransQueueId(lesStockTransQueueId);
            if (exist != null) {
        ...
        //如果是套机，需要拆成子件计算库存
        List<StockHolder> parts = getPartsOfBOM(stockTransaction.getSku(),
        ...
        if (!InvSection.W10.equalsIgnoreCase(itemProperty)) {
            section = invSectionDao.getInvSection(stockTransaction.getExternalSecCode(),
        ...
        if ("S".equalsIgnoreCase(stockTransaction.getMark()) || !isRelease.equals(1)) {
            ...
            List<InvStockLock> locks = invStockLockDao
                .getNotReleasedByRefNo(stockTransaction.getCorderSn());
            if (locks.size() <= 0) {
            ...
            for (StockHolder part : parts) {
                ...
                List<InvStockLock> locksBySkuWhCode = getLocksBySkuWhCode(locks, part.getSku(),
                    section.getWhCode());
        ...
        if (stockTransactions.size() > 0) {
            qty = invStockTransactionDao.insertAll(stockTransactions);
            if (qty > 0 && lesStockTransQueueId != null) {
                try {
                    ...
                    stockTransactionExistDao.insert(stockTransactionExist);
                } catch (Exception e) {
    ...
    public boolean updateBusinessStatus(Integer id, Integer businessProcessStatus) {
        return invStockTransactionDao.updateBusinessProcessStatus(id, businessProcessStatus) > 0;
    }
    ...
    public void processForUpdateStock() {
        while (true) {
            List<InvStockTransaction> stockTransactions = invStockTransactionDao.getByProcessStatus(InvStockTransaction.PROCESS_STATUS_INIT);
            if (stockTransactions.size() <= 0) {
                break;
            }
...
//        TransactionStatus status = transactionManagerStock.getTransaction(def);
        try {
            ...
            if (isDelay == 1)
                isDelay = 0;
            int effectNum = invStockTransactionDao.updateProcessStatus(stockTransaction.getId(),
...
//            transactionManagerStock.rollback(status);
    ...
    public void processForGenerateStockAgeInOut() {
        while (true) {
            List<InvStockTransaction> stockTransactions = invStockTransactionDao.getByProcessStatus(InvStockTransaction.PROCESS_STATUS_UPDATE_STOCK_DOWN);
            if (stockTransactions.size() <= 0)
                break;
            for (InvStockTransaction stockTransaction : stockTransactions) {
                long startTime = System.currentTimeMillis();
...
//            transactionManagerStock.rollback(status);
    ...
    private void updateToDelay(Integer stockTransactionId, Integer isDelay, String message) {
        try {
            invStockTransactionDao.updateToDelay(stockTransactionId, isDelay, message);
        } catch (Exception e) {
    ...
    private InvStockInOut buildStockAgeInOut(InvStockTransaction stockTransaction) {
        ...
        InvSection section = invSectionDao.getBySecCode(sec_code);
    ...
    public void processForDelay() {
        List<InvStockTransaction> stockTransactions = invStockTransactionDao.getByIsDelay(1);
        for (InvStockTransaction stockTransaction : stockTransactions) {
    ...
    public void setStockService(OrderCenterStockServiceImpl stockService) {
        this.stockService = stockService;
    }

    public void setInvStockTransactionDao(StockInvStockTransactionService invStockTransactionDao) {
        this.invStockTransactionDao = invStockTransactionDao;
    }
...
//    }
    ...
    public void setStockAgeService(OrderCenterStockAgeServiceImpl stockAgeService) {
    ...
    public void setTransferLineService(OrderCenterTransferLineServiceImpl transferLineService) {
        this.transferLineService = transferLineService;
    }

    public void setInvMachineSetDao(StockInvMachineSetService invMachineSetDao) {
    ...
    public void setStockTransactionExistDao(StockStockTransactionExistService stockTransactionExistDao) {
        this.stockTransactionExistDao = stockTransactionExistDao;
    }
