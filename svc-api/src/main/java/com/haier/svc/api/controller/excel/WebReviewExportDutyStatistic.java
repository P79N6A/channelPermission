...
public class WebReviewExportDutyStatistic extends WebReviewExcelExportTemplate implements
                                                                              Serializable {
    private static Logger log              = LogManager
                                                   .getLogger(WebReviewExportDutyStatistic.class);
    private static final long serialVersionUID = -2451093586990768406L;

    public void toExcel(List<DutyStatistic> list, HttpServletRequest request, int length, String f,
        ...
        for (int j = 0; j < a; j++) {
            ...
            try {
                String[][] str = new String[][] { { "责任位", "未处理条数", "中间结果条数", "最终结果条数", "工单处理完成率" } };
                ...
                for (int i = 1, min = (list.size() - j * length + 1) > (length + 1) ? (length + 1)
                    ...
                    DutyStatistic duty = list.get(length * (j) + i - 1);
                    int index = 0;
                    row = sheet.createRow(i);
                    HSSFCell cell = row.createCell(index++);
                    row.setHeight((short) 300);//设置行高
                    cell.setCellValue(new HSSFRichTextString(duty.getQuestion1Level1()));//责任位1
