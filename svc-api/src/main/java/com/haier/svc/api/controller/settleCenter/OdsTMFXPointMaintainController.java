...
@Controller
@RequestMapping(value = "/pointMaintain")
public class OdsTMFXPointMaintainController {
    private static Logger     log = LogManager.getLogger(OdsTMFXPointMaintainController.class);

    @Resource
    private OdsTMFXPointMaintainService odsTMFXPointMaintainService;

    @GetMapping("page")
    public String query(){
        return "settleCenter/pointMaintain";
    }


    @RequestMapping(value ="getList")
    @ResponseBody
    public String getList(OdsTMFXPointMaintain param, Integer page, Integer rows) {
        if (page == null) {
            page = 1;
        }
        rows = rows == null ? 50 : rows;
        PagerInfo pager = new PagerInfo(rows, page);
        JSONObject jsonObject = odsTMFXPointMaintainService.paging(param, pager);
    ...
    public String getLogInfo( HttpServletRequest request){
        String logInfo = odsTMFXPointMaintainService.getLogInfo();
        return logInfo;
    }
    ...
    @ResponseBody
    public Object insert(HttpServletRequest request, OdsTMFXPointMaintain odsTMFXPointMaintain) {
        ServiceResult<JSONObject> result = new ServiceResult<>();
        HttpSession session = request.getSession();
        String userName = (String) session.getAttribute("userName");

        try {
            if(StringUtils.isEmpty(odsTMFXPointMaintain.getId())){
                odsTMFXPointMaintain.setCreateBy(userName);
                odsTMFXPointMaintain.setDeleteTab("N");
                odsTMFXPointMaintain.setAuditState("B");
                result = odsTMFXPointMaintainService.insert(odsTMFXPointMaintain);
            }else {
                result = odsTMFXPointMaintainService.update(odsTMFXPointMaintain);
            }
    ...
    public Object delBatch(HttpServletRequest request,String ids) {
        ...
        try {
            ...
            result = odsTMFXPointMaintainService.delBatch(list);
    ...
    public String ImprotExcel(HttpServletRequest request,@RequestParam(value="file")MultipartFile file) {
        ...
        try {
            ...
            serviceResult = odsTMFXPointMaintainService.checkInfo(excelInfo.getBobyInfo());
            if(serviceResult.getSuccess()){
                serviceResult = odsTMFXPointMaintainService.execExcel(excelInfo.getBobyInfo(),userName);
            }else {
        ...
        try {
            if ("B".equals(audit)){
                ...
                for (String id:idArray){
                    OdsTMFXPointMaintain opm = new OdsTMFXPointMaintain();
                    opm.setId(id);
                    opm.setAuditState("F");
                    opm.setAuditBy(userName);
                    opm.setAuditTime(new Date());
                    odsTMFXPointMaintainService.update(opm);
                }
            }
            if ("F".equals(audit)){
                //财务审核
