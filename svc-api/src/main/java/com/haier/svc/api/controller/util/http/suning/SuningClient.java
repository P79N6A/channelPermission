...
@Component
public class SuningClient extends AbstractHaierHttpClient {
    private static final Logger log = LogManager.getLogger(SuningClient.class);
    ...
    public void doCloseOrders(String data) {
        ...
        for (SuningOrderDetail detail:details) {
            ...
            if (StringUtil.isEmpty(sku)) {
                throw new JobException("SNYG关闭订单同步异常：商品物料编码不能为空");
            }
            Products product = productsService.getBySku(sku);
            if (product == null) {
                Salesettings sale = salesettingsService.findByWhere(sku);
                if (sale == null) {
                    log.info("SNYG关闭订单"+order.getOrderCode()+"同步异常：套装SKU查询失败;sku：" + sku);
                    continue;
//                            throw new JobException("SNYG关闭订单同步异常：套装SKU查询失败;sku：" + sku);
                }
                //获取套装内商品
                ...
                for (int i = 0; i < saleNum; i++) {
                    ...
                    if (saleProduct == null) {
                        log.info("SNYG关闭订单"+order.getOrderCode()+"同步异常：商品对应的sku不匹配;sku：" + sku);
                        continue;
                    }
            ...
            log.info("SNYG取消订单同步php返回结果==>" + returnData);
            list.clear();
        }
    }
...
//            SuningApiClient client = new SuningApiClient("http://open.suning.com/api/http/sopRequest", "458e39b7a6c3d198ca1483a9405fb33c", "7c805c40af42c49ac908b71052a608a0", method, req);
            ...
            log.info("苏宁" + method + "请求报文：" + req);
            log.info("苏宁" + method + "返回报文：" + content);
