...
@Controller
@RequestMapping(value = "historyles")
public class HistoryOrderOntimeRateReverseLesReportController {
    private static org.apache.log4j.Logger logger    = org.apache.log4j.LogManager
            .getLogger(HistoryOrderOntimeRateReverseLesReportController.class);
    ...
    @RequestMapping(value = { "/getKpiOrderOntimeRateReverseLesReportList" }, method = { RequestMethod.GET })
    String getOrderOntimeRateReverseList(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            logger.error("[order][orderworkflow]获取历史逆向及时率网单列表时发生未知错误", e);
        }
        return "order/orderOntimeRateReverseLesReportList";
    ...
    @RequestMapping(value = { "/getKpiOrderOntimeRateReverseLesReportDetailList" }, method = { RequestMethod.POST })
    String getOrderOntimeRateReverseReportDetailList(HttpServletRequest request,
        ...
        try {
            ...
            logger.error("[histery][orderworkflow]获取及时率网单明细列表时发生未知错误", e);
        }
        return "order/orderOntimeRateReverseReportLesDetailListHistory";
    }
    ...
    private String getKeyCommissioner(Long region, List<OrderWorkflowRegion> regionList,
        ...
        if (regionList.size() == 0) {//防止每次都去查询行政区列表
            List<OrderWorkflowRegion> tempRegionList = orderWorkflowRegionService.getOwfRegion();
    ...
    public List<Map<String, Object>> getOntimeRateReverseDetailList(String repairId,
        ...
        Map<String, Map<String, Object>> productCateMap = this
                .getProductCate();
        for (Map<String, Object> resultMap : list) {
            ...
            //翻译地区
            resultMap.put("area", getAreaTrade((Long) resultMap.get("region"), "1"));
            //翻译库位
            resultMap.put("sCode", getStorageName((String) resultMap.get("sCode")));
            //翻译转运库位
            resultMap.put("tsCode", getStorageName((String) resultMap.get("tsCode")));
            //翻译工贸
            resultMap.put("trade", getAreaTrade((Long) resultMap.get("region"), "2"));
            //翻译退换货处理状态
            ...
            resultMap.put("isDiffMoney",
                    new BigDecimal(resultMap.get("opMoney").toString())
                            ...
                            : "是");
            //顶级品类
            resultMap.put("superCateId", this
    ...
    public Map<Integer, OrderWorkflowRegion> getRegionMap() {
        ...
        if (null == regionMap) {
            ...
            try {
                List<OrderWorkflowRegion> list = orderWorkflowRegionService.getOwfRegion();
    ...
    public String getStorageName(String id) {
        String storageName = this.getStorageName().get(id);
        if (null != storageName) {
            return storageName;
        }
        return "";
    ...
    public Map<String, String> getStorageName() {
        ...
        if (null == storageMap) {
            ...
            try {
                List<Map<String, String>> storageList = shopOrderWorkflowsService.getStorages();
                for (Map<String, String> map : storageList) {
                    storageMap.put((String) map.get("code"), (String) map.get("name"));
                }
            } catch (Exception e) {
                logger.error("查询库位名称出错", e);
            }
     ...
     * 翻译区域和工贸
     */
    private String getAreaTrade(Long region, String flag) {
        OrderWorkflowRegion owfRegion = this.getRegionMap().get(region.intValue());
        if (null != owfRegion) {
            if ("1".equals(flag)) {
