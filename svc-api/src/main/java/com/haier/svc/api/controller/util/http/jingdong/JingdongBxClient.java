 ...
 */
@Component
public class JingdongBxClient extends AbstractHaierHttpClient {
    private static final Logger log = LogManager.getLogger(JingdongBxClient.class);
    public final static String METHOD_ORDER_DETAIL_GET = "jingdong.pop.order.get";



    @Value("${jingdongbx.url}")
    private String serverUrl;
    @Value("${jingdongbx.appKey}")
    private String appKey;
    @Value("${jingdongbx.appSecret}")
    private String appSecret;
    @Value("${jingdongbx.accessToken}")
    ...
    public OrdersQueue conventOrdersQueue(String respStr) {
        ...
        try {
            ...
            ordersQueue.setSource("JDHEBXGQ");
        } catch (Exception e) {
            e.printStackTrace();
            throw new JobException("京东海尔冰箱旗舰店解析JSON异常");
        }
        return ordersQueue;
    }
    ...
    public String excuteMehtod(String method, TreeMap<String, String> formparams) {
        ...
        try {
            ...
            log.info("京东海尔冰箱旗舰店" + method + "请求报文：" + JSON.toJSONString(formparams));
            log.info("京东海尔冰箱旗舰店" + method + "返回报文：" + content);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return content;
