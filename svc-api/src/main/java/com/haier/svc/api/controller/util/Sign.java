...
public class Sign {
    ...
    public static String buildParamss(Map<String, Object> params, String paramKey) {
        String toVerify = "";
        String theKey = paramKey;
        // 将Map排序
        if (!(params instanceof SortedMap)) {
        ...
        try {
            ...
            String sign = MD5Encoders(toVerify + theKey, "utf-8");
            return sign;
        } catch (Exception e) {
            throw new RuntimeException("failed to build params", e);
        }
    ...
    public final static String MD5Encoders(String s, String charset) {
        try {
            byte[] btInput = s.getBytes(charset);
            MessageDigest mdInst = MessageDigest.getInstance("MD5");
            mdInst.update(btInput);
