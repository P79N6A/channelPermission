...
@Component
public class GomeClient extends AbstractHaierHttpClient {
	private static final Logger log = LogManager.getLogger(GomeClient.class);
	...
	// 查询多个订单
	public  String getOders(OrderPageable orderPageable) {
		Map<String, String> formparams = new TreeMap<String, String>();
//		setParams(orderPageable);
		formparams.put("startDate",orderPageable.getStartDate());
		formparams.put("pageSize", orderPageable.getPageSize()+"");
		formparams.put("endDate",orderPageable.getEndDate());
		formparams.put("pageNo",orderPageable.getPageNo()+"");
		formparams.put("method",METHOD_ORDERS_GET);
		return excuteMehtod(formparams);
	}
	
	//查询订单并分页
	public Iterator<String> getIterator(final OrderPageable orderPageable){
		final List<String> orderinfoList = new ArrayList<String>();
...
//				setParams(orderPageable);
				...
				formparams.put("endDate",orderPageable.getEndDate()); 
				formparams.put("method",METHOD_ORDERS_GET);
				formparams.put("pageNo",Integer.parseInt(String.valueOf((pageNo + 1)))+"");
				String resJson=excuteMehtod(formparams);
				orderinfoList.clear(); 
                if(resJson.contains("\"orders\"")){ 
                 orderinfoList.add(resJson);
               }
	...
	public OrdersQueue conventOrdersQueue(String respStr) {
		...
		try {
			...
			gomeQueue.setSource(ShopStockEnum.GMZX.getSource());
		} catch (Exception e) {
			throw new JobException("GOME解析JSON异常");
		}
		return gomeQueue;
	}
