  ...
  //方法二(推荐):
	...
	function onQuery(pageIndex){
		...
		if(!isNum($( "#lockQty" ).val())){
			...
			$('#list').html("正在加载...");
			$.ajax({
				url: "/memStockLock/queryStockLockWDListPage",   // 提交的页面
				...
				success: function(data) {
				 	var datagridData_orderForecastGoal = {
		          		    'data': {
		          		        'records':
		          		        	data.obj.rowList,'totalCount':data.obj.pager.rowsCount
		          		            }
		        	  } 
					 $('#datagrid_orderForecastGoal').datagrid('loadData',datagridData_orderForecastGoal);
				}
			});
