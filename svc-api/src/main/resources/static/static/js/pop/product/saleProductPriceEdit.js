...
function DeletesAuto() {
    $.ajax({
        type: 'POST',
        async: false,
        url: '/pop/product/deletePriceDetailAtuo',
        data: {
            saleid: gloid
        },
        success: function (data, textStatus) {
            location.href = "/pop/product/saleProductPrice";
        },
            ...
            if (b) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'POST',
                        async: false,
                        url: '/pop/product/deletePriceDetail',
                        data: {
                            id: rows[i].id
                        },
                        success: function (data, textStatus) {
...
function Copy() {
    ...
    $.ajax({
        type: 'POST',
        async: false,
        url: '/pop/product/addPriceDetail',
        data: {
            saleId: gloid,
...
function SearchCounty() {
    ...
    $.ajax({
        type: 'POST',
        url: '/pop/product/priceDetailCountList',
        data: {
...
function SetCodeValue3() {
    ...
    Timejiao(pricestarttime, priceendtime, 0);
    if (timefalg > 0) {
        autoAlt("价格适用时间重复！");
        return;
    }
...
function SetCodeValue4() {
    ...
    Timejiao(pricestarttime, priceendtime, uid);
    if (timefalg > 0) {
        autoAlt("价格适用时间重复！");
        return;
    }
    ...
    $.ajax({
        type: 'POST',
        async: false,
        url: '/pop/product/updatePriceDetail',
        data: {
            id: uid,
            priceStartTime: pricestarttime,
            priceEndTime: priceendtime,
...
function Timejiao(start, end, uid) {
    $.ajax({
        type: 'POST',
        async: false,
        url: '/pop/product/checkPriceTime',
        data: {
            id: uid,
            saleid: gloid,
            startTime: start,
