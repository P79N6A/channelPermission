    ...
    $('#gridView').datagrid('getPager').pagination({
        ...
        onSelectPage: function (pageNo, pageSize) {
            ...
            $.ajax({
                type: 'POST',
                async: true,
                url: '/pop/commission_product/commission_productListF',
                data: {
...
function typesku(productTypeId) {
    $.ajax({
        type: 'POST',
        async: true,
        url: '/pop/product/producttypesListsku',
        data: {
            id: productTypeId
        },
        success: function (data, textStatus) {
        ...
        $.messager.confirm('提示', '您确定要删除这一行吗?', function (b) {
            if (b) {
                $.ajax({
                    type: 'POST',
                    async: false,
                    url: '/pop/commission_product/deletecommission_product',
                    data: {
                        id: rows1[0].id
                    },
                    success: function (data, textStatus) {
...
function SetCodeValue4() {
    ...
    var url = '/pop/commission_product/updatecommission_product';
    ...
    if (id == "") {
        url = '/pop/commission_product/addcommission_product';
    }
...
function jiaoyan() {
    ...
    var policyYear = $('#policyYear').val();
    var id = $('#id').val();
    var url = '/pop/commission_product/jiaoyancommission';
    $.ajax({
        type: 'POST',
        async: false,
        url: url,
...
$("#importBtn_orderList").click(function () {
    ...
    $("#exportForm").attr("action", "/pop/commission_product/exportCommission_product");
    $("#exportForm").submit();
...
$("#importdemo").click(function () {
    ...
    $("#exportForm").attr("action", "/pop/commission_product/exportdemo");
    $("#exportForm").submit();
...
function importZhjj() {
    if ($("#file").val() == null || $("#file").val() == "") {
        ...
        $("#fileForm").ajaxSubmit({
            type: 'post',
            url: '/pop/commission_product/importcommission_product',
                ...
                if (data.msg.substring(0, 2) == "重复") {
                    $.messager.confirm('提示', '物料编码重复是否覆盖?', function (b) {
                        if (b) {
                            ...
                            $("#fileForm").ajaxSubmit({
                                type: 'post',
                                url: '/pop/commission_product/importcommission_product1',
                                success: function (data) {
                                    $.messager.progress('close');
