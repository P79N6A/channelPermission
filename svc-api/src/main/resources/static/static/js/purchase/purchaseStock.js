...
$('#export').click(function(){
    ...
    $.each(checkedItems, function(index, item){
        exportData.push(item.cnStockSyncsId);
    });
    ...
    $.messager.confirm('确认', '确定要导出吗？', function(r){
        if (r){
            $('#filterForm').attr("action", '/purchaseStock/exportPurchaseStock');
            $('#filterForm').submit();
        }
    });
});
...
$('#exportall').click(function(){
    ...
    $("#cnStockSyncsId_save").val($("#cnStockSyncsId").val());
    ...
    $.messager.confirm('确认','确定要全部导出吗？', function(r){
        if (r){
            $('#filterForm').attr("action", '/purchaseStock/exportPurchaseStockList');
            $('#filterForm').submit();
        }
    });
});
...
$('#search').click(function () {
    ...
    //生成grid
    datagrid = $('#datagrid').datagrid({
        url : "/purchaseStock/findPurchaseStockList",
        fit:true,
        pagination: true,
        pageSize:100,
        pageList:[100,200,300],
        ...
        rownumbers: true,
        queryParams: {
            cnStockSyncsId:$("#cnStockSyncsId_save").val(),
            status:status,
