...
$('#revoke').click(function(){
    ...
    if(revokeData==null||revokeData.length==0){
        $.messager.alert('错误','请至少选择一份状态为待内部审核的订单！','error');
        return;
    }
    $.messager.confirm('确认','确定要撤销吗？（只撤销状态为待内部审核的订单）', function(r){
        if (r){
            jQuery.ajax({
                url: "/purchase/revokeOrderList.html",   // 撤销的页面
...
$("#review").click(function(){
    if(datagrid){
        ...
        if(reviewData==null||reviewData.length==0){
            $.messager.alert('错误','请至少选择一行状态为待产品部审核的行！','error');
        }
        else{
            $("#reviewDiv").show();
            $("#reviewDiv").window('open');
        }
    }else{
...
$('#passReview').click(function(){
    ...
    if(flag){
        jQuery.ajax({
            url: "/purchase/reviewT2OrderDepart",   // 提交的页面
            type: "POST",                   // 设置请求类型为"POST"，默认为"GET"
            data:{
            ...
            success: function (data) {
                ...
                if(!data.success) {
                    $.messager.alert('错误','审核失败，请重试','error');
