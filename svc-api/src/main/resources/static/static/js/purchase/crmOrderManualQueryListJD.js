...
$("#pushCrm").click(function(){
    ...
    $.each(checkedItems, function(index, item){
        if(item.flow_flag_name!="未提交"){
            statusValid=false;
        }
    });
    if(statusValid==false){
        $.messager.alert("错误",  "只有未提交的订单能执行该操作");
        return false;
    }
    var openData = new Array();
    $.each(checkedItems, function(index, item){
        openData.push(item.wp_order_id);
    });
    if(openData==null||openData.length==0){
        $.messager.alert('错误','请选择需要提交的行！','error');
        return;
    }
    $.messager.confirm('确认','确定要提交吗？', function(r){
        if(r){
            jQuery.ajax({
                url: "/CrmOrderManual/pushCrmCrmOrder",
                type: "POST",
                ...
                success: function (data) {
                    if(data.success == true){
                        ...
                        $.messager.alert('提交失败', data.message, 'error');
                        $("#datagrid").datagrid('reload');
                    }
                }
            });
...
var importFile = function(){
    ...
    $('#importForm').ajaxSubmit({
        url: '/CrmOrderManual/importOrderManualJD',
        type: 'post',
        success: function(data){
            ...
            if(data.success == true){
                ...
                if(data.message.length > 100){
                    ...
                    $.messager.alert('导入失败', '<a href="javascript:alert(errMessage);">点击查看详情</a>', 'error');
                }else{
