    ...
    $('#gridView').datagrid('getPager').pagination({
        ...
        onSelectPage: function (pageNo, pageSize) {
            ...
            $.ajax({
                type: 'POST',
                async: true,
                url: '/eop/Salesettings/commission_productListF',
                data: {
        ...
        $.messager.confirm('提示', '您确定要删除这一行吗?', function (b) {
            if (b) {
                $.ajax({
                    type: 'POST',
                    async: false,
                    url: '/eop/Salesettings/deletecommission_product',
                    data: {
                        id: rows1[0].id
                    },
                    success: function (data, textStatus) {
...
function SetCodeValue4() {
    ...
    var url = '/eop/Salesettings/updatecommission_product';
    if (settingName == "") {
        autoAlt("套装设置名称不能为空");
        return false;
    }
    ...
    if (id == "") {
        url = '/eop/Salesettings/addcommission_product';
    }
