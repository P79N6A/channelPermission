...
import com.haier.afterSale.util.*;
...
protected String vomSynOrderEaiUrl;
	...
	private Logger				 logger			   = LogManager.getLogger(VOM3WOrderModel.class);
	...
	/**
	 * 取消订单接口
	 * @param cancelOrderRequire
	 * @return
	 */
	public VOMOrderResponse cancelOrderInfo(VOMCancelOrderRequire cancelOrderRequire) {
		String contentXml = getVomFContentXml(cancelOrderRequire);
		logger.info("-------------------调用VOM取消订单接口contentXml：" + contentXml);
		StringBuffer sb = new StringBuffer();
		sb.append("CBS").append(DateFormatUtil.getCurrentDateWithFormat("yyyyMMddHHmmss"))
...
//			sb.append("<remark><![CDATA[" + vomSynOrderRequire.getRemark() + "]]></remark>");// 备注
		...
		sb.append("<remark1>FC,HB</remark1>");// 备用字段 --不填
...
//		String aesContent = AESUtil.Decrypt(content, "KeLy8g7qjmnbgWP1").replaceAll("\n", "");
	...
	//用户取消订单
	public static String getVomFInterXml(String content, String notifyid) throws Exception {
		VomInterData vomInterData = new VomInterData();
		List<VomInterData> list = new ArrayList<VomInterData>();
		// notifyId
		vomInterData.setNotifyid(notifyid);
