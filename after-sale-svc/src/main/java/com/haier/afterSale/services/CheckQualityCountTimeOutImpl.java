 ...
 * 一次质检超时计算实现类
 * 
 * @Filename: CheckQualityCountTimeOutImpl.java
 * @Version: 1.0
 * @Author: yaoyu
 * @Email: yaoyu@ehaier.com
 *
 */
public class CheckQualityCountTimeOutImpl implements ICountTimeOut {
    ...
    public void countTimeOut(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> map : dataList) {
            //计算超时
            this.countTimeOutEachOne(orsIdSet, hpFirstAddTime, hpTime, realCalendar, mustCalendar,
    ...
    public void countTimeOutWithFilter(List<Map<String, Object>> dataList) {
        ...
        for (Map<String, Object> map : dataList) {
            ...
            hpFirstAddTime = NumberParseUtil.parseLong(map.get("hpFirstAddTime"));
            if (hpFirstAddTime == 0) {
                continue;
            }
    ...
    private void countTimeOutEachOne(Set<Long> orsIdSet, Long hpFirstAddTime, Long hpTime,
        ...
        mustCalendar.setTimeInMillis(hpFirstAddTime * 1000l);
        //HP一次质检在向hp推送成功后48H内完成
        mustCalendar.add(Calendar.DAY_OF_YEAR, 2);

        resultMap = new HashMap<String, Object>();
        map.put(ReportType.CHECKQUALITY, resultMap);
        //应完成时间
