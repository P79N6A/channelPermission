...
public class EStoreStockModel {
    ...
    @Autowired
    private InvStoreRegionsService invStoreRegionsDao;
    @Autowired
    private StockInvMachineSetService invMachineSetDao;
    @Autowired
    private InvStoreLockService invStoreLockDao;
    private DataSourceTransactionManager    transactionManagerStock;
    @Autowired
    private InvBaseStoreService invBaseStoreDao;
    @Autowired
    private OrderThirdCenterItemService orderThirdCenterItemService;
    ...
    public Integer acceptStoreStock(List<InvStoreSynch> storeList) {
        ...
        for (int i = 0; i < storeList.size(); i = i + 500) {
            if (temp <= 500) {
                count = invStoreSynchDao.batchInsert(storeList.subList(i, storeList.size()));
            } else {
                count = invStoreSynchDao.batchInsert(storeList.subList(i, i + 500));
            }
    ...
    public Stock getBySkuAndStoreCode(String sku, String storeCode, boolean lockFlag,
        ...
        List<InvStore> storeList = invStoreDao.getByStoreCodesAndSku(sku, storeCode,
            InvSection.W10);
        Integer qty = 0;
    ...
    public InvStore getBySkuAndStoreCode(String sku, Integer regionId, Integer qty,
        ...
        ServiceResult<ItemBase> itemBaseService = orderThirdCenterItemService.getItemBaseBySku(sku);
        ...
        if (basStoresList == null || basStoresList.size() <= 0) {
            log.warn("没有查询到门店信息,regionId[" + regionId + "]");
            return null;
        }
        //获取门店编码
        ...
        if (isTimely) {
            ...
            try {
                storeList = invStoreDao.getByStoreCodesAndSku(sku, storeCodes, itemProperty);
            } finally {
        ...
        if (lockFlag) {
            ...
            for (InvStore entity : storeList) {
                ...
                if (qty.intValue() <= tempQty.intValue()) {
                    log.warn("从门店找到合适库存，sku[" + sku + "]regionId[" + regionId + "]qty[" + qty
    ...
    public InvStore getBySkuAndStoreCodeForLevel(String sku, Integer streetId, Integer qty,
        ...
        ServiceResult<ProductsNew> productBySku = orderThirdCenterItemService.getProductBySku(sku);
        if (!productBySku.getSuccess()) {
            log.error("根据sku获取产品对象发生异常,sku=" + sku);
            return null;
        }
        ProductsNew product = productBySku.getResult();
        if (product == null) {
            log.error("系统不存在此物料,sku=" + sku);
            return null;
        }
        ServiceResult<ItemBase> itemBaseBySku = orderThirdCenterItemService.getItemBaseBySku(sku);
        if (!itemBaseBySku.getSuccess()) {
            log.error("根据物料sku取得物料基本信息发生异常,sku=" + sku);
            return null;
        }
        ...
        List<String> storeCodeByStreet = sgFlagShipStoreAuthorizationDao.getStoreCodeByStreet(
        ...
        if (storeCodeByStreet == null || storeCodeByStreet.size() <= 0) {
            log.error("没有查询到门店信息,streetId[" + streetId + "]getBrandId[" + product.getBrandId()
        ...
        if (lockFlag) {
            ...
            for (InvStore entity : storeList) {
                ...
                if (qty.intValue() <= tempQty.intValue()) {
                    log.warn("从门店找到合适库存，sku[" + sku + "]streetId[" + streetId + "]qty[" + qty
                             + "]storeCode[" + entity.getStoreCode() + "]");
                    return entity;
                } else {
                    if (maxStore == null) {
    ...
    public List<String> getStoreSkuByCityId(Integer cityId) {
        ...
        //查询启用的区县编码
        List<InvStoreRegions> storeRegionsList = invStoreRegionsDao.getByCityId(cityId,
    ...
    public List<ProductBase> getAllOnSaleProducts() {
        ServiceResult<List<ProductBase>> result = orderThirdCenterItemService.getAllOnSaleProducts();
    ...
    public ServiceResult<String> frozeStockQty(String sku, String storeCode, String refNo,
        ...
        //检查单据号是否已经冻结
        List<InvStoreLock> storeLocks = invStoreLockDao.getNotReleased(refNo, sku, storeCode,
            InvSection.W10);
        ...
        try {
            ...
            for (StockHolder stockHolder : stockHolders) {
                ...
                List<BaseStock> storeList = StockBizHelper.regroupStockQtyBySku(invBaseStoreDao,
                    invMachineSetDao, baseStock);
    ...
    private InvBaseStore frozenStoreStock(String refNo, String sku, String storeCode, String source,
                                          ...
                                          String optUser) {
        InvBaseStore baseStore = invBaseStoreDao.getByItemPropertyForUpdate(sku, storeCode,
            itemProperty);
        if (baseStore == null) {
        ...
        int effectRows = invBaseStoreDao.updateQtyForFrozen(baseStore);
        if (effectRows < 1) {
        ...
        int effect = invStoreLockDao.insert(stockLock);
        if (effect <= 0) {
    ...
    private void releaseFrozenQty(String refNo, String sku, String storeCode, Integer releaseQty,
        ...
        for (StockHolder stockHolder : stockHolders) {
            List<InvStoreLock> storeLocks = invStoreLockDao.getNotReleased(refNo, stockHolder.getSku(),
    ...
    private void releaseStoreQty(String sku, String storeCode, Integer qty, String itemProperty,
        ...
        for (BaseStock store : storeList) {
            ...
            InvStore invStore = invStoreDao.getByStoreCodeAndSku(store.getSku(), storeCode,
                itemProperty);
            if (invStore == null) {
                invStore = new InvStore();
                invStore.setStoreCode(storeCode);
    ...
    private BasStores getStoresByStoreCodeNoLimit(String storeCode, Integer storeStatus) {
        ...
        SgStore sg = sgStoreDao.getSgStore(storeCode, null, storeStatus);
        if (sg == null) {
            return null;
        }
        BasStores basStores = new BasStores();
    ...
    private List<BasStores> getStoresByRegionId(Integer regionId, Integer storeStatus,
        ...
        List<SgStore> sgList = sgStoreDao.getSgStoreList(regionId, BasStores.STOR_TYPE_1,
    ...
    */
    ...
    private void releaseStoreLockQty(String sku, String storeCode, String refNo, Integer releaseQty,
                                     String optUser) {
        InvStoreLock storeLock = invStoreLockDao.getNotReleasedForUpdate(refNo, sku, storeCode,
            InvSection.W10);
        if (storeLock == null) {
            ...
            if (notReleaseQty >= releaseQty) {
                Integer effectNum = invStoreLockDao.updateReleaseQty(storeLock.getId(), releaseQty,
    ...
    private InvBaseStore releaseBaseStoreQty(String sku, String storeCode, Integer releaseQty,
        ...
        InvBaseStore baseStore = invBaseStoreDao.getStore(sku, storeCode, InvSection.W10);
        if (baseStore == null) {
            throw new BusinessException("店铺库存不存在");
        }
        Integer effectNum = 0;
        if (calTotalStockFlag) {
            effectNum = invBaseStoreDao.releaseStockQtyAndFrozenQty(baseStore.getId(), releaseQty);
        } else {
            effectNum = invBaseStoreDao.releaseFrozenQty(baseStore.getId(), releaseQty);
        }

        if (effectNum < 1) {
            throw new BusinessException("扣减店铺库存占用数量失败");
        }
        return invBaseStoreDao.getStore(sku, storeCode, InvSection.W10);

    }

    public List<InvStoreRegions> getInvStoreRegionByRegionId(Integer regionId, Integer status) {
        return invStoreRegionsDao.getByRegionId(regionId, status,
    ...
    public List<Stock> getEStoreBySkuAndStoreCodeList(String sku, List<String> storeCodeList) {
        ...
        if (storeCodeList != null && storeCodeList.size() > 0) {
            if (storeCodeList.size() <= 50) {
                invStores = invStoreDao.getEStoreBySkuAndStoreCodeList(sku,
                ...
                for (int i = 0; i < storeCodeList.size(); i = i + 50) {
                    if (temp <= 50) {
                        List<InvStore> tempInvStore = invStoreDao
                        ...
                        List<InvStore> tempInvStore = invStoreDao.getEStoreBySkuAndStoreCodeList(
                            sku,
            ...
            invStores = invStoreDao.getEStoreBySkuAndStoreCodeList(sku, null, InvSection.W10);
        }
    ...
    public List<Stock> getEStoreBySkuListAndStoreCode(List<String> skuList, String storeCode) {
        ...
        if (skuList != null && skuList.size() > 0) {
            if (skuList.size() <= 50) {
                invStores = invStoreDao.getEStoreBySkuListAndStoreCode(
                ...
                for (int i = 0; i < skuList.size(); i = i + 50) {
                    if (temp <= 50) {
                        ...
                        List<InvStore> tempInvStore = invStoreDao.getEStoreBySkuListAndStoreCode(
            ...
            invStores = invStoreDao.getEStoreBySkuListAndStoreCode(null, storeCode, InvSection.W10);
        }
        if (invStores == null || invStores.size() <= 0) {
            return null;
        }
    ...
    public List<Stock> getEStoreIncrement(Date startTime) {
        ...
        List<InvStore> invStores = invStoreDao.getChangedStockQty(startTime);
        if (invStores == null || invStores.size() <= 0) {
            return null;
        }
        List<Stock> stocks = new ArrayList<Stock>();
   ...
   /* public void setInvMachineSetDao(InvMachineSetDao invMachineSetDao) {
    ...
    public void setInvStoreLockDao(InvStoreLockDao invStoreLockDao) {
        this.invStoreLockDao = invStoreLockDao;
    }
    ...
    public void setInvStoreRegionsDao(InvStoreRegionsDao invStoreRegionsDao) {
        this.invStoreRegionsDao = invStoreRegionsDao;
    }
   ...
   /* public void setMctStoreProductsDao(MctStoreProductsDao mctStoreProductsDao) {
    ...
    public void setSgFlagShipStoreAuthorizationDao(SgFlagShipStoreAuthorizationDao sgFlagShipStoreAuthorizationDao) {
        this.sgFlagShipStoreAuthorizationDao = sgFlagShipStoreAuthorizationDao;
    }
...
*/
    ...
    public Integer orderLockByRefNo(String sku, String storeId, String scode, Integer type,
        ...
        Integer lockQty = invStockOrderLockDao.findLockQtyByScodeAndStoreCodeAndSku(scode, storeId,
            sku, refNo);
        return lockQty;
    }
