...
import com.haier.stock.service.StockCenterEInvoiceService;
...
import com.haier.stock.util.SerializedPhpParser;
import com.haier.stock.util.SmsTemplateConst;
import com.haier.stock.util.StrTools;
import com.haier.stock.util.UnmarshalXmlUtil;
...
public class InvoiceModel {
    ...
    @Autowired
    private ShopTaoBaoGroupsService taShopTaoBaoGroupsService;
    @Autowired
    ...
    @Autowired
    private InvoiceElectricLogsService invoiceElectricLogsService;
    @Autowired
    ...
    @Autowired
    private InvoiceElectricSyncLogsService invoiceElectricSyncLogsService;
    @Autowired
    ...
    @Autowired
    private InvoiceChangeLogsService invoiceChangeLogsService;
    ...
    private void eInvoiceSendPhp(Invoices invoices, InvoiceElectricLogs eInvoiceLog) {
        switch (eInvoiceLog.getType()) {
            case 1:
                ...
                invoicesService.update(invoices);
                break;
            case 2:
    ...
    public void invalidInvoice(Invoices invoices) throws Exception {
        ...
        if (invoices.getElectricFlag() == 1) {
            OrderProductsNew orderProducts = orderProductsNewService.get(invoices.getOrderProductId());
            ...
            invoiceElectricLogsService.insertLog(eInvoiceLog);
    ...
    public void editInvoice(Invoices invoices, Integer status, Integer orderId) throws Exception {
        ...
        OrdersNew orders = ordersNewService.get(orderId);
    ...
    public void createInvoice() {
        //1、获取待开发票队列
        List<InvoiceQueue> list = invoiceQueueService.getBySuccess(InvoiceQueue.SUCCESS_NO);
    ...
    public void createInvoiceThread(List<InvoiceQueue> list) {
        try {
            ...
            for (InvoiceQueue item : list) {
                ...
                //获取网单信息
                OrderProductsNew op = orderProductsNewService.get(opId);
                if (op == null) {
                ...
                OrdersNew order = ordersNewService.get(orderId);
                if (order == null) {
                ...
                if (order.getOrderStatus().equals(OrderStatus.OS_CANCEL.getCode())) {
                    ...
                    invoiceQueueService.updateAfterProccess(item);
                    continue;
                }
                //网单取消不需要开票
                ...
                //获取订单发票信息
                MemberInvoices mi = memberInvoicesService.getByOrderId(orderId);
                if (mi == null) {
                ...
                try {
                    //创建发票信息
                	invoicesService.insertInvoice(invoice);
                    //更新网单信息
                    op.setIsMakeReceipt(5);//开票中
                    orderProductsNewService.updateAfterCreateInvoice(op);
                    //记录开票操作日志
                    OrderOperateLogs log = this.AssemblyOrderOperateLog(order, op, "发票创建通知",
    ...
    private Invoices AssemblyInvoiceInfo(OrdersNew order, OrderProductsNew op, MemberInvoices mi) {
        ...
        //已开过票，不需要开票
        List<Invoices> invList = invoicesService.getByOrderProductId(op.getId());
    ...
    public BigDecimal getInvoiceAmount(String cOrderSn, boolean flag) {
        ...
        //获取网单信息
        OrderProductsNew op = orderProductsNewService.getByCOrderSn(cOrderSn);
        if (op == null) {
    ...
    public BigDecimal calInvoiceAmount(OrdersNew order, OrderProductsNew op, boolean flag) {
        ...
        if (OrderType.TYPE_GROUP_ADVANCE.getValue().equals(order.getOrderType())
            && op.getActivityId() > 0) {
            //从促销活动表中获取价格
            ProductActivities pa = productActivitiesService.get(op.getActivityId());
            BigDecimal paPrice = this.getPriceInActivities(pa);
            if (paPrice != null) {
                amount = paPrice.multiply(new BigDecimal(op.getNumber()));
            }
            //如果尾款订单使用了礼券，还得减去礼券的金额
            GroupOrders go = groupOrdersService.getByDepositOrderId(Integer.parseInt(order.getId()));//订单尾款订单匹配表
            if (go != null) {
                //尾款网单
                OrderProductsNew top = orderProductsNewService.getByCOrderSn(go.getTailCOrderSn());
                if (top != null && top.getCouponAmount().compareTo(BigDecimal.ZERO) > 0) {
                    //尾款网单使用了优惠券，从总计中扣除
                    amount = amount.subtract(top.getCouponAmount());
                }
    ...
    public void syncToInvoiceSystem() {
        try {
            //1、获取待开发票队列
            List<Invoices> list = invoicesService.getSyncInvoiceList(500);//php原值300   开票不用太快 平时500，双十一改成1000
            if (list == null || list.size() == 0) {
    ...
    public void syncToInvoiceSystemCreate(Invoices invoices) {
        ...
        try {
            ...
            if (InvoiceConst.COMMON_CORDER_TYPE.equals(invoices.getCOrderType())) {//普通网单
                ...
                memberInvoices = memberInvoicesService.getByOrderId(orderProducts.getOrderId());
            } else if (InvoiceConst.DIFF_CORDER_TYPE.equals(invoices.getCOrderType())) {//差异网单
                Order2ths order2ths = order2thsService.get(invoices.getDiffId());
                memberInvoices = memberInvoicesService.get(order2ths.getMemberInvoiceId());
                mobile = order2ths.getMobile1();
                consignee = order2ths.getConsignee();
                industryName = order2ths.getIndustryName();
            } else { //专项开票
                Order4Invoices order4Invoices = order4InvoicesService.get(invoices.getDiffId());
                memberInvoices = memberInvoicesService.get(order4Invoices.getMemberInvoiceId());
            ...
            if (InvoiceConst.ELECTRIC_SWITCH
                ...
                memberInvoicesService.update(memberInvoices);
            }
            //并发时，过滤同步操作数据库
            ...
            if (invoices.getElectricFlag().equals(1)) {
                ...
                if (InvoiceConst.COMMON_CORDER_TYPE.equals(invoices.getCOrderType())) {
                    order = ordersNewService.get(orderId);
                } else {
                    order = new OrdersNew();
                ...
                try {
                    if (result == null) {
                        ...
                        if (InvoiceConst.COMMON_CORDER_TYPE.equals(invoices.getCOrderType())) {
                            // 普通网单
                            OrderProductsNew op = orderProductsNewService.get(invoices.getOrderProductId());
                            if (op != null) {
                                op.setIsMakeReceipt(InvoiceConst.MR_STATE_MAKING);
                                orderProductsNewService.update(op);
                            }
                            ...
                            // 专项开票
                            Order4Invoices order4Invoices = order4InvoicesService.get(invoices
                                .getDiffId());
                            if (order4Invoices != null) {
                                order4Invoices.setIsReceipted(InvoiceConst.R_RECEIPTING);
                                order4InvoicesService.update(order4Invoices);
                            }
                        }
                    ...
                    shopInvoiceApiLogsService.updateByInvoiceIdAndType(apilogs);
        ...
        try {
            ...
            if (memberInvoices.getElectricFlag().equals(1) || invoices.getElectricFlag().equals(1)) {
                ...
                if (!(invoices.getElectricFlag() != null && invoices.getElectricFlag().equals(1))) {
                    ...
                    invoicesService.update(invoices);//后面还会执行更新
                }
                OrdersNew order = null;
                if (InvoiceConst.COMMON_CORDER_TYPE.equals(invoices.getCOrderType())) {
                    order = ordersNewService.get(orderId);
    ...
    private void writeInvoiceElectricLogs(Integer invoiceId, Integer type, Integer orderProductId,
        ...
        log.setSmsFlag(0);
        invoiceElectricLogsService.insertLog(log);
    ...
    public void syncToInvoiceSystemInvalidOld(Invoices invoices) {
        ...
        if (InvoiceConst.COMMON_CORDER_TYPE.equals(invoices.getCOrderType())) {//普通网单
            ...
            //获取订单信息
            OrdersNew order = ordersNewService.get(orderProducts.getOrderId());
            if (order == null) {
    ...
    private InvoiceElectricLogs eInvoiceGenerateOld(Invoices invoice, OrdersNew order) {
        ...
        if (order.getProvince() != null && !order.getProvince().equals(0)) {
            provincename = regionsService.get(order.getProvince()).getRegionName();
        }
        if (order.getCity() != null && !order.getCity().equals(0)) {
            cityname = regionsService.get(order.getCity()).getRegionName();
        }
        if (order.getRegion() != null && !order.getRegion().equals(0)) {
            regionname = regionsService.get(order.getRegion()).getRegionName();
        }
    ...
    private InvoiceElectricLogs saveInvoiceElectricLogs(Invoices invoice, String data, Integer type) {
        ...
        log.setLastMessage("");
        log.setSmsFlag(0);
        n = invoiceElectricLogsService.insertLog(log);
        //        } else {
        //            //            log.setAddTime((int) (new Date().getTime() / 1000));
        //            log.setOrderProductId(invoice.getOrderProductId());
        //            log.setCOrderSn(invoice.getCOrderSn());
    ...
    private InvoiceApiLogs saveInvoiceApiLogs(Invoices invoice, String data, Integer type,
        ...
        InvoiceApiLogs log = shopInvoiceApiLogsService.getByInvoiceIdAndType(invoice.getId(), type);
        int n = 0;
        if (log == null) {
            ...
            log.setLastMessage(message);
            log.setIsSuccess(flag);
            n = shopInvoiceApiLogsService.insert(log);
        } else {
            log.setOrderProductId(invoice.getOrderProductId());
            log.setCOrderSn(invoice.getCOrderSn());
            //            log.setInvoiceId(invoice.getId());
            ...
            log.setLastMessage(message);
            log.setIsSuccess(flag);
            n = shopInvoiceApiLogsService.updateByInvoiceIdAndType(log);
        }
        if (n < 1) {
            return null;
        }
    ...
    public Invoices getInvoicesByCorderSn(String cOrderSn) {
        return invoicesService.getByCorderSn(cOrderSn);
    }
    ...
    public Map<String, Object> searchInvoicesInfoByCOrderSn(String cOrderSn) {
        return invoicesService.searchInvoicesInfoByCOrderSn(cOrderSn);
    }
    ...
    public OrdersNew searchOrdersInfoByCOrderSn(String cOrderSn) {
        ...
        if (op == null)
            return null;
        return ordersNewService.get(op.getOrderId());
    }
        ...
        switch (type) {
            case 1://开票
                ...
                if (doc1 == null) {
                    ...
                    invoiceElectricLogs.setLastMessage(message[0]);
                    invoiceElectricLogsService.updateByInvoiceIdAndType(invoiceElectricLogs);
                    invoicesService.update(invoice);
                    return false;
                }
    ...
    private boolean eInvoiceUpdateOld(Invoices invoice, Integer type, Map<String, String> attMap) {
        ...
        switch (type) {
            case 1://开票
                ...
                if (invoice.getCOrderType().equals(InvoiceConst.COMMON_CORDER_TYPE)) {//普通网单
                    OrderProductsNew op = orderProductsNewService.get(invoice.getOrderProductId());
                    if (op != null) {
                        ...
                        OrdersNew order = ordersNewService.get(op.getOrderId());
                        ...
                        List<InvoiceElectricSyncLogs> iesllist = invoiceElectricSyncLogsService
                            .getByCOrderSnAngStatusType(invoice.getCOrderSn(), statusType);
                        if (iesllist == null || iesllist.size() < 1) {
                        	invoiceElectricSyncLogsService.insert(invoiceElectricSyncLogs);
                        }
                    ...
                    Order4Invoices order4Invoice = order4InvoicesService.get(invoice.getDiffId());
                    if (order4Invoice != null) {
                        ...
                        order4InvoicesService.update(order4Invoice);
                    }
                ...
                MemberInvoices mInvoices = memberInvoicesService.getByOrderId(orderId);
                if (mInvoices != null) {
                    mInvoices.setIsLock(1);//开票成功后设置锁，不让修改信息
                    memberInvoicesService.update(mInvoices);
                }

                InvoiceSAPLogs invoiceSAPLogs = OrderBizHelper.sapGenerate(shopInvoiceSAPLogsService,
                		orderProductsNewService, invoice, 0, this.getPathByName("/upfiles/sapblacklist.txt"));
                boolean sendSap = false;
                if (invoiceSAPLogs != null) {
                    int n = shopInvoiceSAPLogsService.insert(invoiceSAPLogs);
                    if (n < 1) {
            ...
            case 2://作废
                ...
                if (attMap.get("status").equals("3") || attMap.get("status").equals("4")) {//原php invalidateType=1   新java  status=3冲红     status=2作废   status=1开票   status=4被红冲
                    ...
                    if (invoice.getCOrderType().equals(InvoiceConst.COMMON_CORDER_TYPE)) {
                        ...
                        Order2ths order2th = order2thsService.get(invoice.getDiffId());
                ...
                invoiceSAPLogs = OrderBizHelper.sapGenerate(shopInvoiceSAPLogsService, orderProductsNewService,
        ...
        if (!InvoiceConst.SMSFLAG && sourceSMSFlag) {
            ...
            InvoiceElectricLogs ielog = invoiceElectricLogsService.getByInvoiceIdAndType(
                invoice.getId(), type);
            if (ielog != null) {
                if (ielog.getSmsFlag().equals(0) && userMobile.length() == 11
                    ...
                    ielog.setSmsFlag(1);
                    invoiceElectricLogsService.update(ielog);
                }
            }
        }
        return true;
    ...
    private boolean orderRepairsCancelInvoice(Integer orderProductId) {
        ...
        List<OrderRepairsNew> orderRepairsList = orderRepairsNewService.getByOrderProductId(orderProductId);
        ...
        for (int i = 0; i < orderRepairsList.size(); i++) {
            ...
            orderRepairsNewService.update(orderRepairsList.get(i));
    ...
    public String returnSuffix(Integer opid) {
        ...
        OrderProductsNew orderProducts = orderProductsNewService.get(opid);
        if (orderProducts != null) {
            OrdersNew order = ordersNewService.get(orderProducts.getOrderId());
    ...
    private void insertInvoiceElectric2Out(Integer invoiceId, String sendType) {
        ...
        List<InvoiceElectric2Out> ie2olist = invoiceElectric2OutService
    ...
    public String getCustomerCode(int productID) {
        List<Invoices> invoives = invoicesService.getByOrderProductId(productID);
        if (invoives == null || invoives.isEmpty())
            return "A69";
        List<String> codes = customerCodesService.getCustomerCode(invoives.get(0).getInvoiceTitle());
    ...
    public String saveMemberInvoices(Integer id, Integer invoiceType, String invoiceTitle,
        ...
        try {
            // 根据Id获得发票信息
            MemberInvoices memberInvoices = this.memberInvoicesService.get(id);
            ...
            List<OrderProductsNew> orderProductsList = this.orderProductsNewService
            ...
            if (idsList.size() > 0) {
                ...
                orderProductsList = this.orderProductsNewService.getByIds(idsList);
                if (orderProductsList != null && orderProductsList.size() > 0) {
                    for (OrderProductsNew eachOrderProducts : orderProductsList) {
                        // 已开票的场合
            ...
            try {
                ...
                if (paramMemberInvoices.getInvoiceType().equals(1)) {
                    ...
                    if (orderProductsList != null && orderProductsList.size() > 0) {
                        for (OrderProductsNew eachOrderProducts : orderProductsList) {
                            ...
                            this.orderProductsNewService.updateMakeReceiptType(eachOrderProducts);
                        }
                    }
                    ...
                    MemberInvoices queryMemberInvoices = this.memberInvoicesService
                    ...
                    if (queryMemberInvoices != null) {
                        ...
                        this.memberInvoicesService.update(paramMemberInvoices);
                    } else {
                ...
                if (!oldmemberInvoices.getInvoiceTitle().equals(
                    ...
                    invoiceChangeLogs.setRemark("发票抬头：由“" + oldmemberInvoices.getInvoiceTitle()
                                                ...
                                                + "”");
                    invoiceChangeLogsService.insert(invoiceChangeLogs);
                }
    ...
    public void syncInvoiceInvalidInfoFromEInvoiceSystemOld() {
        try {
            //1、获取待开发票队列
            List<Invoices> list = invoicesService.getInvoiceInvalidNotEndList(100);
            if (list == null || list.size() == 0) {
                ...
                for (Invoices invoices : list) {
                    ...
                    try {
                        ...
                        if (paramMap != null) {
                            ...
                            TransactionStatus status = transactionManager.getTransaction(def);
                            try {
                                if (OrderBizHelper.updateInvoiceInfo(invoices, 2, invoicesService,
                                		orderProductsNewService, shopInvoiceSAPLogsService,
                        ...
                        if (paramMap != null) {
                            ...
                            try {
                                if (OrderBizHelper.updateInvoiceInfo(invoices, pushType,
                                		invoicesService, orderProductsNewService, shopInvoiceSAPLogsService,
                                    this.getPathByName("/upfiles/sapblacklist.txt"))) {
