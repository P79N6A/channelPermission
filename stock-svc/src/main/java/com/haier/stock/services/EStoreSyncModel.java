...
public class EStoreSyncModel {
    ...
    @Autowired
    private StockCenterItemServiceImpl                  itemService;
    @Autowired
    private StockCenterEStoreInfoServiceImpl            eStoreInfoServiceImpl;
    ...
    public String storeStockSync(final InvStoreSynch invStoreSynch) {
        ...
        InvStore invStore = invStoreService.getByStoreCodeAndSku(invStoreSynch.getSku(),
                ...
                public boolean process() throws Exception {
                    ...
                    for (StockHolder holder : stockHolderList) {
                        ...
                        if (newStockQty < 0)
                            newStockQty = 0;
                        //查找库存
                        InvBaseStore invBaseStore = invBaseStoreService.getByItemPropertyForUpdate(
                        ...
                        //更新InvBaseStock
                        if (isInsert) {
                            invBaseStoreService.insert(invBaseStore);
                        } else {
                            invBaseStoreService.updateStockQty(invBaseStore.getId(),
                                convertDate(invStoreSynch.getUpdateTime()).getTime() / 1000,
                                newStockQty);
                        }
                        //计算销售库存数,包括涉及到的套机计算
                        ...
                        List<BaseStock> storeList = StockBizHelper
                            .regroupStockQtyBySku(invBaseStoreService, stockInvMachineSetService, baseStock);
    ...
    private void updateStoreSynch(Integer storeSyncId, Integer status, String message) {
        ...
        invStoreSynchService.update(storeSynch);
    }
    ...
    */
    ...
    public boolean realTimeGetStoreQty(String storeCode, String sku, String itemProperty) {
        try {
            ...
            ServiceResult<EStoreStock> result = eStoreInfoServiceImpl.getByStoreCode(storeCode, sku,
                itemProperty);
            if (result == null || !result.getSuccess()) {
                LOGGER.info("同步EC库存失败：storeCode=" + storeCode + ",sku=" + sku);
                return false;
            }
            EStoreStock stock = result.getResult();
            if (stock == null) {
                LOGGER.info("同步EC库存失败，查询返回结果为空：storeCode=" + storeCode + ",sku=" + sku);
                return false;
            }
            ...
            LOGGER.error("查询EC库存失败sku=" + sku + ", storeCode=" + storeCode + ",itemProperty="
                        + itemProperty);
            return false;
        }
        return true;
    ...
    public void setInvStoreDao(InvStoreDao invStoreService) {
    ...
    public void setInvStoreSynchDao(InvStoreSynchDao invStoreSynchService) {
        this.invStoreSynchService = invStoreSynchService;
    }
