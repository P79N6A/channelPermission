...
import com.haier.stock.util.OrderSnUtil;
import com.haier.stock.util.PayCenterJsonUtils;
import com.haier.stock.util.SignUtil;
...
public class OrderModel {
    ...
    @Autowired
    private OrdersAttributesService ordersAttributesService;
    @Autowired
    ...
    @Autowired
    private OrderShippedQueueService orderShippedQueueService;
    @Autowired
    private InvoiceQueueService invoiceQueueService;
    @Autowired
    private InvoicesReadyService invoicesReadyService;
    @Autowired
    private OrderRepairsNewService orderRepairsNewService;
    @Autowired
    private HpReservationDateLogsService hpReservationDateLogsService;
    @Autowired
    private HpSignTimeInterfaceService hpSignTimeInterfaceService;
    @Autowired
    private CmtCommentOrderProductsService cmtCommentOrderProductsService;
    ...
    public ServiceResult<Boolean> deliveryOrderLine(OrdersNew order, OrderProductsNew orderProduct,
        ...
        if (orderProduct.getStockType() != null
                        ...
                        if (StringUtil.isEmpty(orderProduct.getTsCode())) {
                            ...
                            if (!StringUtil.isEmpty(orderProduct.getSCode())
                                ...
                                if (OrderSnUtil.SCODE_MAP.containsKey(np.getTCCode())) {
                                    ...
                                    OrderWorkflowRegion region = orderWorkflowRegionService
                                            .get(order.getRegion());
            ...
            //            if (StringUtil.isEmpty(orderProduct.getTsCode())) {
            //                OrderWorkflowRegion region = orderWorkflowRegionService.get(order.getRegion());
        ...
        try {
            ...
            //更新订单全流程监控信息 - 出库时间
            OrderWorkflows wf = shopOrderWorkflowsService.getByOrderProductId(orderProduct.getId());
            if (wf != null) {
                ...
                if (orderProduct.getStockType() != null
                    ...
                    shopOrderWorkflowsService.updateAfterCnShipped(wf);
                } else {
                    shopOrderWorkflowsService.updateAfterLesShipped(wf);
                }
            } else {
                log.warn(
            ...
            orderShippedQueueService.insert(queue);
            ...
            if (invoicing) {
                ...
                if (n > 0) {
                    OrderOperateLogs invlog = this.getOrderOperateLog(order, orderProduct, "系统",
                            "创建发票队列", "添加网单到发票队列完成");
                    shopOrderOperateLogsService.insert(invlog);
                }
            }
    ...
    public int insertInvoice(OrdersNew order, OrderProductsNew orderProduct){
        ...
        List<InvoiceQueue> inv_queue_list = invoiceQueueService
                .getByOrderProductId(orderProduct.getId());
    ...
    public ServiceResult<Boolean> transferOrderLineIn(OrdersNew order, OrderProductsNew orderProduct,
        ...
        //检查是否开票
        InvoicesReady ir = invoicesReadyService.getByOrderProductId(orderProduct.getId());
        if (ir != null) {
            OrderOperateLogs log = this.getOrderOperateLog(order, orderProduct, "系统", "LES出库",
                    "转运开票失败：已经存在开票队列记录");
            shopOrderOperateLogsService.insert(log);
        ...
        //获取退货单
        OrderRepairsNew repair = orderRepairsNewService.getValidByOrderProductId(orderProduct.getId());
        ...
        try {
            ...
            invoicesReadyService.insert(ir);
            //开票日志
    ...
    public Boolean saveHpReservationDateRelation(OrderProductsNew orderProduct, String remark,
    	...
    	try {
    		...
    		hpReservationDateLogsService.insert(hpReservationDateLogs);
    		//订单操作日志
    ...
    public OrderProductsAttributes getByOrderProductId(Integer orderProductId) {
        return orderProductsAttributesService.getByOrderProductId(orderProductId);
    }
    /**
     * 根据网单号获取订单
     * @param orderSn
    ...
    public OrderRepairsNew getValidByOrderProductId(Integer orderProductId) {
        return orderRepairsNewService.getValidByOrderProductId(orderProductId);
    }
    
    public List<OrderRepairsNew> getOrderRepairsByOrderProductId(Integer orderProductId) {
        return orderRepairsNewService.getByOrderProductId(orderProductId);
    }
    ...
    public boolean updateOrderWorkflowNetPointAcceptTime(String orderSn, Date acceptTime,
        ...
        OrderWorkflows orderWorkflow = shopOrderWorkflowsService.getByOrderProductId(orderProducts.getId());
        ...
        try {
            ...
            if (!(netPointAcceptTime != null && netPointAcceptTime > 0)) {
                shopOrderWorkflowsService.updateNetPointAcceptTime(orderWorkflow.getId(),
    ...
    public boolean updateOrderWorkflowNetPointShipTime(String orderSn, Date shipTime,
        ...
        try {
            ...
            if (!(netPointShipTime != null && netPointShipTime > 0)) {
                shopOrderWorkflowsService.updateNetPointShipTime(orderWorkflow.getId(),
    ...
    public boolean updateOrderWorkflowUserAcceptTime(HpSignTimeInterface hpSignTimeInterface,
        ...
        if ("HR".equalsIgnoreCase(hpSignTimeInterface.getWwwMark())) {
            ...
            hasInfo = hpSignTimeInterfaceService.selectByTbNoAndLbx(hpSignTimeInterface);
            if (hasInfo != null && hasInfo.getDataStatus() != null
                ...
                hpSignTimeInterfaceService.addCountBySkuAndLbx(hpSignTimeInterface);
                return true;
            } else {
        ...
        if ("HR".equalsIgnoreCase(hpSignTimeInterface.getWwwMark())) {
            if (hasInfo == null) {
                ...
                hpSignTimeInterfaceService.insert(hpSignTimeInterface);
            }
        }
        return true;
    }
    public boolean completeCloseOrderProductOper(String corderSn, Date signTime,
        ...
        try {
            ...
            if (orderProductsNew.getCPaymentStatus() != 200) {//未付款的等待付款后自动签收
                ...
                //更新全流程表的网单关闭时间
                if (orderWorkflows != null) {
                    shopOrderWorkflowsService.updateFinishTime(orderWorkflows.getId(),
                            now.getTime() / 1000);
                }
                ...
                if (orders.getMemberId() > 0
                    ...
                    cmtCommentOrderProductsService.insert(cmtCommentOrderProducts);
                }
            } else {
    ...
    public void insertErrorHpSignTimeInterface(HpSignTimeInterface hpSignTimeInterface) {
        ...
        if (hpSignTimeInterface.getStatus() != null && hpSignTimeInterface.getStatus() == 3
            ...
            HpSignTimeInterface hasInfo = hpSignTimeInterfaceService
    ...
    public boolean payCenterFallBack(OrderProductsNew orderProduct, OrdersNew order, String repairSn,
    		String[] message) {
    	try {
    		OrderRepairsNew ors = orderRepairsNewService.getByRepairSn(repairSn);
    		if (ors == null) {
    			message[0] = "根据退换货单号查询不到信息,单号:" + repairSn;
    			return false;
    		}
    		List<OrderRepairHPRecordsNew> hpList = orderRepairHPRecordsnNewService.getByRepairId(ors.getId());
    ...
    public List<OrderShippedQueue> getOrderShippedQueueForProcess(Integer topX) {
        return orderShippedQueueService.getListToProcess(topX);
    }
    /**
     * 根据id列表，获取网单列表
     * @param ids
