...
import com.haier.stock.eai.finance.pushReturnInfoToGVS.ObjectFactory;
import com.haier.stock.eai.finance.pushReturnInfoToGVS.OutType;
import com.haier.stock.eai.finance.pushReturnInfoToGVS.PushReturnInfoToGVS;
import com.haier.stock.eai.finance.pushReturnInfoToGVS.PushReturnInfoToGVS_Service;
...
@Service("inToGVSModel")
public class TransferDefectiveProductsInToGVSModel {

    private Logger logger = LogManager.getLogger(TransferDefectiveProductsInToGVSModel.class);
    @Value("${wsdlPath}")
    ...
    private String wsdlFile = "PushReturnInfoToGVS.wsdl";
    @Autowired
    ...
    private void transfer(InvThTransaction transaction) {
        ...
        request
            .setMATNR(transaction.getSubCount() == null || transaction.getSubCount() <= 1 ? transaction
                .getSku() : transaction.getMainSku());//物料编码
        ...
        // 原始网单号
        if (orderProducts.getSplitFlag() .equals(OrderProductsNew.SPLITFLAG_SPLIT_NEW)) {
