...
import com.haier.stock.service.HopStockService;
...
//    private EisLesStatusItemService eisLesStatusItemService;
    ...
    @Autowired
    private HopStockService hopStockService;
///*
//    @Autowired
...
//    }
    ...
    public void releaseCancelCloseStock() {

        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DAY_OF_MONTH, -2);
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String lockTime = dateFormat.format(calendar.getTime());

        ServiceResult<List<InvStockLock>> stockResult = hopStockService.getNoReleaseByLockTime(
            lockTime, 1000);
        if (stockResult == null || !stockResult.getSuccess()) {
            log.error("查询没有释放的库存[商城、海鹏、基地库]失败");
            return;
        }
        List<InvStockLock> invStockLockList = stockResult.getResult();
        if (invStockLockList == null || invStockLockList.isEmpty()) {
            log.info("不存在没有释放的库存[商城、海鹏、基地库]");
            return;
        }
        for (InvStockLock invStockLock : invStockLockList) {
            String refNo = invStockLock.getRefno();
            if (StringUtil.isEmpty(refNo, true)) {
                log.error("网单号为空");
                continue;
            }
            String sku = invStockLock.getSku();
            Integer lockQty = invStockLock.getLockQty();
            Integer realeaseQty = invStockLock.getRealeaseQty();
            ServiceResult<OrderProductsNew> result = orderService.getOrderProductByCOrderSn(refNo);
            if (result == null || !result.getSuccess()) {
                log.error("查询网单号[" + refNo + "]对应的网单失败");
                continue;
            }
            OrderProductsNew orderProducts = result.getResult();
            if (orderProducts == null) {
                log.info("不存在网单号[" + refNo + "]对应的网单");
                continue;
            }
            if (OrderProductStatus.CANCEL_CLOSE.getCode().intValue() != orderProducts.getStatus()
                .intValue()) {
                log.info("网单号[" + refNo + "]，物料[" + sku + "]，对应的网单状态[" + orderProducts.getStatus()
                         + "]不是取消关闭");
                continue;
            }
            ServiceResult<Boolean> releaseResult = hopStockService.releaseFrozenStockQty(sku,
                lockQty - realeaseQty, refNo, InventoryBusinessTypes.RELEASE_BY_ZBCC);
            if (releaseResult == null || !releaseResult.getSuccess()) {
                log.error("取消销售释放[商城、海鹏、基地库]库存失败:"
                          + (releaseResult != null ? releaseResult.getMessage() : ""));
                continue;
            }
            log.info("网单号[" + refNo + "]，库位[" + invStockLock.getSecCode() + "]，物料[" + sku
                     + "]，已释放库存");
        }
    }
}
